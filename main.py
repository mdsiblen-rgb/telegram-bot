# FINAL BIG FILE - MONEY BOY - ALL BUGS FIXED - SINGLE FILE
...
def load_db():
    if not os.path.exists(DB_FILE):
        return {"users":{},"withdraws":[],"banned":[],"slider":[{"img":"https://images.unsplash.com/photo-1484480974693-6ca0a78fb36b?w=800","link":"https://t.me/ProtidinerKajBD"},{"img":"https://images.unsplash.com/photo-1506784365847-bbad939e9335?w=800","link":"https://t.me/ProtidinerKajBD"},{"img":"https://images.unsplash.com/photo-1517245386807-bb43f82c33c4?w=800","link":"https://youtube.com/@ProtidinerKajBD"}],"settings":{"app_name":"প্রতিদিনের কাজ BD","ad_zone":"11764581","ad_reward":1,"ad_limit":100,"min_withdraw":1000,"welcome_bonus":60,"daily_bonus":10,"ref_bonus":20,"payment_time":"প্রতিদিন রাত 8PM - 10PM","payment_rules":"ভুল Number দিবেন না","admin_msg_title":"অফিশিয়াল চ্যানেল","admin_msg_desc":"Ads দেখুন Task করুন","my_ad_title":"🔥 স্পেশাল অফার","my_ad_desc":"এডমিন থেকে চেঞ্জ হবে","support_title":"🎧 সাপোর্ট সেন্টার","support_desc":"সমস্যা হলে যোগাযোগ করুন - এডমিন এটা লিখবে","support_extra":"সঠিক তথ্য দিন"},"tasks":[{"title":"YouTube ভিডিও দেখুন","reward":25,"link":"https://youtube.com/@ProtidinerKajBD","btn":"শুরু করুন","color":"#065f46"},{"title":"Telegram Channel Join","reward":10,"link":"https://t.me/ProtidinerKajBD","btn":"Join","color":"#1e40af"},{"title":"Facebook Follow","reward":15,"link":"https://facebook.com","btn":"Follow","color":"#1877F2"},{"title":"Company Task 1","reward":20,"link":"https://t.me/ProtidinerKajBD","btn":"Visit","color":"#7c3aed"},{"title":"Company Task 2","reward":20,"link":"https://t.me/ProtidinerKajBD","btn":"Visit","color":"#0f766e"},{"title":"Company Task 3","reward":20,"link":"https://t.me/ProtidinerKajBD","btn":"Visit","color":"#be123c"}]}
...
.btm{position:fixed;bottom:0;left:50%;transform:translateX(-50%);width:100%;max-width:430px;background:#fff;display:flex;border-top:1px solid #ddd;padding:20px 0 26px;z-index:99}
.btm div{flex:1;text-align:center;color:#94a3b8;font-size:17px;font-weight:800}
.btm div.on{color:#1e40af;background:#e8edff;border-radius:18px;transform:scale(1.25)}
...
<div class="prof"><img src="https://cdn-icons-png.flaticon.com/512/3135/3135715.png" style="width:64px;height:64px;border-radius:50%;background:#fff"><div style="flex:1"><div id="pn" style="font-weight:900;font-size:18px"></div><div>ID: <span id="pid"></span></div><div style="display:flex;gap:6px;margin-top:8px"><input id="en" placeholder="নতুন নাম লিখুন" style="padding:8px;border-radius:10px;border:none;flex:1;color:#000"><button onclick="cn()" style="background:#f59e0b;border:none;padding:8px 12px;border-radius:10px;color:#fff;font-weight:700">Edit</button></div></div></div>
...
function load(){fetch('/api/get_full?id='+uid).then(r=>r.json()).then(d=>{document.getElementById('tb').innerText=d.user.balance;document.getElementById('b').innerText=d.user.balance;document.getElementById('pb').innerText=d.user.balance;document.getElementById('pid').innerText=uid;document.getElementById('uid2').innerText=uid;document.getElementById('pn').innerText=d.user.name;document.getElementById('w').innerText=d.user.ads_watched||0;document.getElementById('pa').innerText=(d.user.ads_watched||0)+' টি';document.getElementById('pt').innerText='৳'+(d.user.today_earn||0);document.getElementById('py').innerText='৳'+(d.user.yesterday_earn||0);document.getElementById('ptw').innerText=d.user.total_withdraw||0;document.getElementById('pr').innerText=(d.user.refer_count||0)+' জন';document.getElementById('rc').innerText=d.user.refer_count||0;document.getElementById('ptot').innerText='৳'+(d.user.total_earn||0);document.getElementById('appN').innerText=d.settings.app_name;document.getElementById('myT').innerText=d.settings.my_ad_title;document.getElementById('myD').innerText=d.settings.my_ad_desc;document.getElementById('adT').innerText=d.settings.admin_msg_title;document.getElementById('adD').innerText=d.settings.admin_msg_desc;document.getElementById('sT').innerText=d.settings.support_title;document.getElementById('sD').innerText=d.settings.support_desc;document.getElementById('sE').innerText=d.settings.support_extra;document.getElementById('ar').innerText=d.settings.ad_reward;document.getElementById('rb').innerText=d.settings.ref_bonus;document.getElementById('rl').innerText='https://t.me/ProtidinerKajBD_bot?start='+uid;let bx=document.getElementById('tasks');bx.innerHTML='';d.tasks.forEach(t=>{bx.innerHTML+='<div class=card><div style=display:flex;justify-content:space-between><span>'+t.title+'</span><b>৳'+t.reward+'</b></div><button class=btn style=background:'+t.color+';margin-top:8px onclick=window.open(`'+t.link+'`)>'+t.btn+'</button></div>'})})}
...
function cn(){let v=document.getElementById('en').value.trim();if(!v){alert('নাম লিখো');return}fetch('/api/update_name?id='+uid+'&name='+encodeURIComponent(v)).then(r=>r.json()).then(x=>{alert(x.msg);load()})}
function copyR(){let t=document.getElementById('rl').innerText;navigator.clipboard.writeText(t).then(()=>alert('কপি হয়েছে ✅')).catch(()=>{let ta=document.createElement('textarea');ta.value=t;document.body.appendChild(ta);ta.select();document.execCommand('copy');ta.remove();alert('কপি হয়েছে ✅')})}
load();
</script></body></html>
"""

ADMIN_HTML = """
<!DOCTYPE html><html><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>Admin</title><style>body{max-width:500px;margin:0 auto;background:#eef6f3;padding:10px;font-family:sans-serif}.c{background:#fff;border-radius:16px;padding:14px;margin:10px}.i{width:100%;padding:10px;border:1px solid #ddd;border-radius:10px;margin-top:4px;box-sizing:border-box}.b{width:100%;background:#0f766e;color:#fff;padding:14px;border-radius:12px;border:none;font-weight:900}</style></head><body>
<div style="background:#0f766e;color:#fff;padding:16px;border-radius:16px"><b>Admin Panel - Final Fixed</b></div>
<div class="c"><b>🎧 সাপোর্ট এডিট (তুমি যা লিখবা তাই ইউজার দেখবে)</b><input id="st" class="i" placeholder="Support Title"><textarea id="sd" class="i" placeholder="Support Desc"></textarea><input id="se" class="i" placeholder="Extra"></div>
<div class="c"><b>📢 Home Text</b><input id="mt" class="i" placeholder="My Ad Title"><input id="md" class="i" placeholder="My Ad Desc"><input id="at" class="i" placeholder="Admin Title"><input id="ad" class="i" placeholder="Admin Desc"></div>
<div class="c"><b>🔗 6 Link Edit</b><input id="l0" class="i" placeholder="Link 1"><input id="l1" class="i" placeholder="Link 2"><input id="l2" class="i" placeholder="Link 3"><input id="l3" class="i" placeholder="Link 4"><input id="l4" class="i" placeholder="Link 5"><input id="l5" class="i" placeholder="Link 6"></div>
<div class="c"><b>⚙️ Setting</b><input id="an" class="i" placeholder="App Name"><input id="ar" class="i" placeholder="Ad Reward"><input id="rb" class="i" placeholder="Ref Bonus"></div>
...
def get_full():
    ...
    if uid not in d['users']:
        d['users'][uid] = {"balance": d['settings']['welcome_bonus'],"ads_watched":0,"name": f"User {uid[-4:]}","total_earn": d['settings']['welcome_bonus'],"today_earn":0,"yesterday_earn":0,"total_withdraw":0,"refer_count":0,"total_withdraw":0}
        save_db(d)
    return jsonify({"user": d['users'][uid],"settings": d['settings'],"tasks": d['tasks'],"slider": d['slider']})
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if uid not in d['users']:
        d['users'][uid] = {"balance": d['settings']['welcome_bonus'],"ads_watched":0,"name": update.effective_user.first_name or f"User {uid[-4:]}","total_earn": d['settings']['welcome_bonus'],"today_earn":0,"yesterday_earn":0,"total_withdraw":0,"refer_count":0}
        ...
        save_db(d)
    kb = [[InlineKeyboardButton("🚀 Open Money Boy App", web_app={"url": f"{SELF_URL}/?id={uid}"})]]
    await update.message.reply_text(f"Welcome {update.effective_user.first_name} 🎉\nBalance: ৳{d['users'][uid]['balance']}", reply_markup=InlineKeyboardMarkup(kb))
...
def run_bot():
    try:
        ...
        print("Money Boy Bot Started - No Error")
        app_bot.run_polling(drop_pending_updates=True)
    except Exception as e:
        print(f"Bot Error: {e} - But Flask will run")
