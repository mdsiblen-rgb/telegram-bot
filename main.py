...
<div class="card"><p>👥 Refer - প্রতি Refer ৳<span id="refR">50</span></p><p id="refLink" style="font-size:11px;background:#f1f5f9;padding:10px;border-radius:8px;word-break:break-all;border:1px dashed #aaa"></p>
...
<div class="card"><h3>🎧 সাপোর্ট - কোম্পানি লিংক</h3>
<button class="btn" style="background:#1e40af" onclick="window.open('https://t.me/ProtidinerKajBD','_blank')">📢 অফিশিয়াল চ্যানেল</button>
<button class="btn" style="background:#FF0000" onclick="window.open('https://youtube.com/@ProtidinerKajBD','_blank')">▶️ YouTube Channel</button>
<button class="btn" style="background:#1877F2" onclick="window.open('https://www.facebook.com/share/1AXw16vWRj/','_blank')">📘 Facebook Page</button>
<button class="btn" style="background:#10b981" onclick="window.open('https://t.me/ProtidinerKajBD','_blank')">💬 Admin Chat</button></div>
</div>
<div id="t-withdraw" class="tab">
<div class="card"><h3>💳 উইথড্র - বিকাশ নগদ</h3><div style="font-size:36px;text-align:center;font-weight:900">৳<span class="bal2">0</span></div><div class="wd-info"><span id="wdMsg">সর্বনিম্ন উইথড্র ৳1000 - আপনার আর <b id="needAmt">245</b> টাকা লাগবে</span></div></div>
<div class="card">
<h4>💳 পেমেন্ট মেথড - বিকাশ + নগদ - FINAL ✅</h4>
<div style="display:flex;gap:12px">
<div onclick="selectMethod('bKash')" id="m-bKash" class="meth sel" style="border-color:#e11d48">
<img src="https://upload.wikimedia.org/wikipedia/commons/b/bd/BKash_Logo.png" style="height:34px;width:auto;display:block;margin:0 auto;object-fit:contain">
...
<img src="https://upload.wikimedia.org/wikipedia/commons/8/87/Nagad_Logo.png" style="height:34px;width:auto;display:block;margin:0 auto;object-fit:contain">
...
<div class="card"><h4>📜 History</h4><div id="wdHistory"><p style="font-size:11px;color:#888">কোনো History নেই</p></div></div>
</div>
...
function selectMethod(m){curMeth=m;document.getElementById('selectedMethod').value=m;document.querySelectorAll('.meth').forEach(e=>{e.classList.remove('sel');});document.getElementById('m-'+m).classList.add('sel');document.getElementById('wNum').placeholder=m+' Personal Number';}
...
<style>body{max-width:700px;margin:0 auto;font-family:Arial;background:#f1f5f9;padding:12px}.card{background:#fff;padding:14px;border-radius:12px;margin:12px 0}input{width:100%;padding:10px;margin:5px 0;border-radius:8px;border:1px solid #ddd;box-sizing:border-box}input[type=color]{height:45px}.btn{width:100%;padding:12px;border:none;border-radius:8px;color:#fff;font-weight:bold;background:#1e40af;cursor:pointer;margin-top:6px}.grid{display:grid;grid-template-columns:1fr 1fr;gap:10px}table{width:100%;font-size:12px;border-collapse:collapse}th,td{border:1px solid #ddd;padding:6px}th{background:#f8fafc}</style></head><body>
<h2 style="text-align:center">🔥 FINAL ADMIN - Anti Sleep Fixed</h2>
<div class="card" style="border:2px solid #22c55e;background:#f0fdf4"><b>✅ Bot Status Fixed</b><br><small>Self-Ping 4m + /health OK. Render Start Command python main.py দিতে হবে। My project খালি Project টা Delete করো। UptimeRobot এ /health Add করো।</small></div>
<div class="card"><h3>🎨 Design + Logo</h3><label>App Name</label><input id="app_name"><label>Logo URL</label><input id="logo_url"><div class="grid"><div><label>Banner</label><input type="color" id="banner_color"></div><div><label>BG</label><input type="color" id="bg_color"></div></div><div class="grid"><div><label>Card</label><input type="color" id="card_bg"></div><div><label>Bal C1</label><input type="color" id="balance_c1"></div></div><div class="grid"><div><label>Bal C2</label><input type="color" id="balance_c2"></div><div><label>Ad Btn</label><input type="color" id="ad_btn"></div></div><div class="grid"><div><label>Wd Btn</label><input type="color" id="wd_btn"></div><div><label>Checkin</label><input type="color" id="checkin_btn"></div></div><label>Notice</label><input id="home_notice"><button class="btn" style="background:#f59e0b;color:#000" onclick="saveDesign()">Save Design</button></div>
<div class="card"><h3>💰 Money</h3><div class="grid"><div><label>Welcome</label><input id="welcome_bonus" type="number"></div><div><label>Ad Reward</label><input id="ad_reward" type="number"></div><div><label>Refer</label><input id="ref_bonus" type="number"></div><div><label>Min WD</label><input id="min_withdraw" type="number"></div><div><label>Daily</label><input id="daily_bonus" type="number"></div><div><label>Zone</label><input id="ad_zone"></div></div><button class="btn" style="background:#10b981" onclick="saveMoney()">Save Money</button></div>
<div class="card"><h3>📊 Stats</h3><div id="stats"></div></div>
<div class="card"><h3>👥 Users</h3><table><thead><tr><th>ID</th><th>Bal</th><th>Ads</th></tr></thead><tbody id="userTable"></tbody></table></div>
<div class="card"><h3>💸 Withdraw - bKash Nagad</h3><table><thead><tr><th>UID</th><th>Method</th><th>Num</th><th>Amt</th></tr></thead><tbody id="wdTable"></tbody></table></div>
<div class="card"><h3>📢 Slider 3.5s</h3><input id="sTitle" placeholder="Title"><input id="sImg" placeholder="Image URL"><input id="sLink" placeholder="Link"><button class="btn" onclick="addSlider()">+ Add</button><div id="sliderList"></div></div>
<script>
let aid=new URLSearchParams(location.search).get('id')||'8807178385';
function loadAll(){fetch(`/api/pro_stats?id=${aid}`).then(r=>r.json()).then(d=>{document.getElementById('app_name').value=d.settings.app_name;document.getElementById('logo_url').value=d.settings.logo_url;document.getElementById('banner_color').value=d.settings.banner_color;document.getElementById('bg_color').value=d.settings.bg_color;document.getElementById('card_bg').value=d.settings.card_bg;document.getElementById('balance_c1').value=d.settings.balance_c1;document.getElementById('balance_c2').value=d.settings.balance_c2;document.getElementById('ad_btn').value=d.settings.ad_btn;document.getElementById('wd_btn').value=d.settings.wd_btn;document.getElementById('checkin_btn').value=d.settings.checkin_btn;document.getElementById('home_notice').value=d.settings.home_notice||'';document.getElementById('welcome_bonus').value=d.settings.welcome_bonus;document.getElementById('ad_reward').value=d.settings.ad_reward;document.getElementById('ref_bonus').value=d.settings.ref_bonus;document.getElementById('min_withdraw').value=d.settings.min_withdraw;document.getElementById('daily_bonus').value=d.settings.daily_bonus;document.getElementById('ad_zone').value=d.settings.ad_zone;document.getElementById('stats').innerHTML=`Users: ${d.total_users}<br>Bal: ৳${d.total_balance}<br>WD: ${d.total_withdraws}`;let ut=document.getElementById('userTable');ut.innerHTML='';d.users.forEach(u=>{ut.innerHTML+=`<tr><td>${u.id}</td><td>৳${u.balance}</td><td>${u.ads}</td></tr>`;});let wt=document.getElementById('wdTable');wt.innerHTML='';d.withdraws.forEach(w=>{wt.innerHTML+=`<tr><td>${w.uid}</td><td>${w.method}</td><td>${w.num}</td><td>৳${w.amt}</td></tr>`;});let sl=document.getElementById('sliderList');sl.innerHTML='';d.slider.forEach((s,i)=>{sl.innerHTML+=`<div style="border:1px solid #ddd;padding:8px;margin:8px 0;border-radius:10px"><img src="${s.img}" style="width:100%;height:70px;object-fit:cover;border-radius:8px"><p>${s.title}</p><button onclick="delS(${i})" style="background:#ef4444;color:#fff;border:none;padding:6px 12px;border-radius:6px">Delete</button></div>`;});});}
...
function addSlider(){let img=document.getElementById('sImg').value;let link=document.getElementById('sLink').value;let title=document.getElementById('sTitle').value;if(!img)return alert('Image URL');fetch(`/api/add_slider?id=${aid}&img=${encodeURIComponent(img)}&link=${encodeURIComponent(link)}&title=${encodeURIComponent(title)}`).then(r=>r.json()).then(d=>{alert(d.msg);loadAll();});}
...
@app.route('/health')
def health(): return jsonify({"status":"ok","bot":"alive"}),200
@app.route('/ping')
...
def daily():
    ...
    return jsonify({"ok":True,"msg":f"✅ ৳{d['settings']['daily_bonus']} Bonus!"})
...
def pro_stats():
    ...
    d=load_db()
    users_list=[{"id":k,"balance":v.get('balance',0),"ads":v.get('ads_watched',0)} for k,v in list(d['users'].items())[-100:]]
    return jsonify({"total_users":len(d['users']), "total_balance":sum([u.get('balance',0) for u in d['users'].values()]), "total_withdraws":len(d['withdraws']), "users":users_list[::-1], "withdraws":d['withdraws'][::-1][:100], "slider":d['slider'], "settings":d['settings']})
...
# ====== FLASK + BOT DUAL MODE - FAILED FIX ======
def run_flask():
    ...
    async def runner():
        while True:
            try:
                ...
                print("✅ Bot Polling Started - আর ঘুমাবে না")
...
if __name__ == '__main__':
    ...
    # gunicorn হলে bot thread এ চলবে
