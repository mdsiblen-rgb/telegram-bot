...
def start(message):
    ...
    if not c.fetchone():
        ...
        if ref_id and ref_id!= user_id:
            ...
            try: bot.send_message(ref_id, f"🎉 নতুন রেফার! ২৫ টাকা পেয়েছেন।")
            except: pass
        conn.commit()
    conn.close()
    bot.reply_to(message, f"স্বাগতম! 👋\n\nআপনার ID: {user_id}\nআপনার লিঙ্ক: https://am-bot-1-v77g.onrender.com/?id={user_id}\n\n1. প্রথমে আমাদের চ্যানেলে জয়েন করুন: {CHANNEL_LINK}\n2. তারপর ওয়েবসাইটে গিয়ে Task Complete করুন, ১০ টাকা পাবেন।\n\nরেফার করলে ২৫ টাকা!")
...
def home(request: Request):
    ...
    <p>Step 1: Join Channel - <a href='{CHANNEL_LINK}' target='_blank'>{CHANNEL_LINK}</a></p>
    <p>Step 2: Click Below</p>
    ...
    <p>Refer Link: https://am-bot-1-v77g.onrender.com/?id={user_id}</p>
    </body></html>
    """
...
def task(user_id: str):
    try:
        ...
        return f"আগে চ্যানেলে জয়েন করুন: {CHANNEL_LINK} অথবা বটকে চ্যানেলে Admin করুন। Error: {e}"
...
def earnings(user_id: str):
    ...
    conn.close()
    return f"<h2>Balance: {bal} TK</h2><form action='/api/withdraw' method='get'><input type='hidden' name='user_id' value='{user_id}'><input name='number' placeholder='Bkash/Nagad Number' required><input name='amount' type='number' placeholder='Amount' required><button>Withdraw Request</button></form><br><a href='/?id={user_id}'>Back Home</a>"
...
def admin_page(admin_id: str):
    if admin_id!= ADMIN_ID: return HTMLResponse("Not allowed. Your ID is not admin", status_code=403)
    conn = sqlite3.connect('database.db')
    c = conn.cursor()
    c.execute("SELECT * FROM withdraws WHERE status='pending'")
    rows = c.fetchall()
    conn.close()
    html = "<h1>Admin Dashboard - Withdraw Requests</h1>"
    for r in rows:
...
def approve(id: int, admin: str):
    if admin!= ADMIN_ID: return "Not admin"
    conn = sqlite3.connect('database.db')
    c = conn.cursor()
