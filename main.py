...
<div id="t-profile" style="display:none">
    <div class="prof">
        ...
        <div style="flex:1">
            ...
            <button onclick="cn()" style="width:100%;background:#f59e0b;color:#fff;padding:10px;border:none;border-radius:10px;margin-top:8px;font-weight:700">✅ নাম + ছবি সেভ করুন</button>
        </div>
    </div>
    <div class="gcard">
        <span>💼 ব্যালেন্স</span>
...
def claim_task():
    ...
    claims = d['users'][uid].get('task_claims', {})
    key = f"{idx}_{today}"

    # Telegram & Facebook one time
    if idx in [1, 2]:
        if str(idx) in claims:
            return jsonify({"msg": "এটা একবারই পাবে ভাই"})
        claims[str(idx)] = today
    else:
        if key in claims:
            return jsonify({"msg": "আজকের জন্য নিয়েছো, কাল আবার"})
        claims[key] = today

    rew = d['tasks'][idx]['reward']
    d['users'][uid]['balance'] += rew
    d['users'][uid]['today_earn'] = d['users'][uid].get('today_earn', 0) + rew
    d['users'][uid]['total_earn'] = d['users'][uid].get('total_earn', 0) + rew
    d['users'][uid]['task_claims'] = claims
    save_db(d)
    return jsonify({"msg": f"৳{rew} যোগ হয়েছে ✅"})
...
def wd():
    ...
    if amt < d['settings']['min_withdraw']:
        return jsonify({"msg": f"Min Withdraw ৳{d['settings']['min_withdraw']}"})
    if d['users'][uid]['balance'] < amt:
        return jsonify({"msg": "Balance কম আছে ভাই"})
    d['users'][uid]['balance'] -= amt
    d['users'][uid]['total_withdraw'] = d['users'][uid].get('total_withdraw', 0) + 1
    d['withdraws'].append({
        "uid": uid,
        "amt": amt,
        "num": num,
    ...
    save_db(d)
    return jsonify({"msg": "Withdraw Request গেছে ✅"})
...
def daily():
    ...
    d['users'][uid]['today_earn'] += d['settings']['daily_bonus']
    d['users'][uid]['total_earn'] += d['settings']['daily_bonus']
    d['users'][uid]['last_daily'] = today
    save_db(d)
    return jsonify({"msg": f"Daily ৳{d['settings']['daily_bonus']} যোগ ✅"})

@app.route('/api/update_profile')
def update_profile():
    uid = request.args.get('id')
    d = load_db()
    name = request.args.get('name', '').strip()[:30]
    av = request.args.get('avatar', '').strip()[:500]
    if name:
        d['users'][uid]['name'] = name
    if av:
        d['users'][uid]['avatar'] = av
    save_db(d)
    return jsonify({"msg": "✅ নাম + ছবি সেভ হয়েছে"})
...
def admin_wd():
    if not is_admin(request.args.get('id')):
        return jsonify([])
    return jsonify(load_db()['withdraws'])
...
def admin_stats():
    if not is_admin(request.args.get('id')):
        return jsonify({})
    d = load_db()
    total = sum([u.get('balance', 0) for u in d['users'].values()])
    return jsonify({
        "total_users": len(d['users']),
        "pending_wd": len(d['withdraws']),
        "total_taka": total
    })
...
def save_all():
    ...
    data = request.json
    d = load_db()
    d['settings']['company_logo'] = data.get('companyLogo') or d['settings']['company_logo']
    d['settings']['app_name'] = data.get('appName') or d['settings']['app_name']
    d['settings']['ad_reward'] = int(data.get('perAd') or 1)
    d['settings']['ad_limit'] = int(data.get('adLim') or 100)
    d['tasks'][0]['reward'] = int(data.get('ytR') or 25)
    d['tasks'][1]['reward'] = int(data.get('tgR') or 10)
    d['tasks'][2]['reward'] = int(data.get('fbR') or 15)
    d['tasks'][3]['reward'] = int(data.get('c1R') or 20)
    d['tasks'][4]['reward'] = int(data.get('c2R') or 20)
    d['tasks'][5]['reward'] = int(data.get('c3R') or 20)
    ...
    d['settings']['my_ad_title'] = data.get('myAdTitle') or d['settings']['my_ad_title']
    d['settings']['my_ad_desc'] = data.get('myAdDesc') or d['settings']['my_ad_desc']
    d['settings']['admin_msg_title'] = data.get('adTitle') or d['settings']['admin_msg_title']
    d['settings']['admin_msg_desc'] = data.get('adDesc') or d['settings']['admin_msg_desc']
    d['settings']['support_title'] = data.get('sT') or d['settings']['support_title']
    d['settings']['support_desc'] = data.get('sD') or d['settings']['support_desc']
    d['settings']['support_extra'] = data.get('sE') or d['settings']['support_extra']
    d['settings']['support_custom'] = data.get('sCustom') or d['settings']['support_custom']
    d['tasks'][0]['link'] = data.get('ytLink') or d['tasks'][0]['link']
    d['tasks'][1]['link'] = data.get('chLink') or d['tasks'][1]['link']
    d['tasks'][2]['link'] = data.get('fbLink') or d['tasks'][2]['link']
    d['tasks'][3]['link'] = data.get('c1') or d['tasks'][3]['link']
    d['tasks'][4]['link'] = data.get('c2') or d['tasks'][4]['link']
    d['tasks'][5]['link'] = data.get('c3') or d['tasks'][5]['link']
    d['settings']['payment_time'] = data.get('payTime') or d['settings']['payment_time']
    d['settings']['payment_rules'] = data.get('payRule') or d['settings']['payment_rules']
    if data.get('s1'): d['slider'][0]['img'] = data.get('s1')
    if data.get('s2'): d['slider'][1]['img'] = data.get('s2')
    if data.get('s3'): d['slider'][2]['img'] = data.get('s3')
    save_db(d)
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        "Welcome ✅ - তোমার বড় ফাইল + খালি জায়গা ফিক্স Live - FINAL BLUE",
