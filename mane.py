...
def load_db():
    ...
    try:
        with open(DB_FILE,'r',encoding='utf-8') as f:
            j=json.load(f)
            for k in default["s"]:
                if k not in j.get("s",{}): j["s"][k]=default["s"][k]
            return j
...
def claim():
    ...
    if tid in u["tasks"]: return jsonify({"ok":False,"msg":"DONE ✅ 24h পরে আবার"})
...
def wd():
    ...
    u["bal"]-=amt; db["w"].append({"id":uid,"amt":amt,"num":request.args.get('num',''),"met":request.args.get('met','bKash'),"time":str(datetime.now())[:16]}); save_db(db); return jsonify({"msg":"Withdraw Success"})
...
@keyframes blk{0%{opacity:1;transform:scale(1)}50%{opacity:0.2;transform:scale(0.6)}100%{opacity:1;transform:scale(1)}}
...
.inp{width:100%;padding:12px;border-radius:10px;background:#0b142d;color:#fff;border:1px solid #2a3a5f;margin-top:8px}
</style></head><body>
<div id="p-home"><div class="hdr"><div class="hdr-top"><div class="hdr-name">👑 <span id="hn">BD</span> ✅</div><img id="hp" class="hdr-pic" src=""></div><div class="bal">৳ <span class="balV">60</span></div><div style="opacity:0.7;font-size:13px">Ads: <span class="adsV">0</span>/100</div></div>
<div class="notice"><div><div style="font-weight:900">অফিসিয়াল নোটিস</div><div style="font-size:12px;opacity:0.8">প্রতিদিন Ads দেখুন - ৳500 পর্যন্ত</div></div><div class="live-btn"><span class="dot"></span>LIVE</div></div>
<div class="slider" id="sl"></div>
<div class="card"><b>🎬 স্পেশাল অফার - বোনাস Ads</b><button class="btn" style="background:#0ea5e9;margin-top:10px" onclick="ads()">ADS দেখুন - ৳2 বোনাস</button></div>
<div id="thome"></div></div>
...
<div id="p-refer" style="display:none"><div class="hdr"><div>Refer - ৳ <span class="balV">60</span></div></div><div class="card"><input id="rlink" class="inp" readonly></div></div>
...
<div id="p-profile" style="display:none"><div class="hdr" style="text-align:center"><div>Profile</div><div class="bal">৳ <span class="balV">60</span></div></div></div>
...
let lv=d.w.length==0?'কেউ Withdraw করেনি':'';d.w.forEach(w=>{lv+=`<div style="padding:5px 0">💸 ${w.id.slice(-4)} - ৳${w.amt}</div>`;});document.getElementById('live').innerHTML=lv;
document.getElementById('rlink').value=location.origin+'/?ref='+d.id;
});}load();
</script></body></html>
"""
...
<style>
body{background:#081028;color:#fff;font-family:sans-serif;padding:10px;max-width:500px;margin:auto;padding-bottom:120px}
.card{background:#132042;border:1px solid #1e2d4f;border-radius:14px;padding:12px;margin:10px 0}
.label{color:#38bdf8;font-size:11px;margin:8px 0 3px 0}
.input{background:#0a1229;border:1px solid #2a3a5f;border-radius:8px;padding:11px;width:100%;color:#fff;margin-bottom:5px}
.btn-save{position:fixed;bottom:10px;left:50%;transform:translateX(-50%);width:95%;max-width:480px;padding:16px;background:linear-gradient(90deg,#22c55e,#16a34a);border:0;border-radius:14px;font-weight:900;color:#fff}
.tab{display:flex;gap:5px;overflow:auto;margin:10px 0}
.tab div{padding:9px 13px;background:#132042;border-radius:8px;white-space:nowrap;cursor:pointer;border:1px solid #1e2d4f;font-size:13px}
.tab div.on{background:#22c55e;color:#000;font-weight:900}
</style></head><body>
<h2 style="text-align:center;color:#22c55e">👑 SUPER ADMIN A-Z</h2>
<div class="tab"><div class="on" id="t1" onclick="tab(1)">Settings</div><div id="t2" onclick="tab(2)">Tasks - Link</div><div id="t3" onclick="tab(3)">Slider</div></div>

<div id="p1">
<div class="card"><div style="font-weight:900">👑 Header</div><div class="label">নাম</div><input class="input" id="admin_name"><div class="label">ছবি Link</div><input class="input" id="admin_pic"></div>
<div class="card"><div style="font-weight:900">📢 Notice</div><div class="label">Title</div><input class="input" id="notice_title"><div class="label">Sub Title</div><input class="input" id="notice_sub"><div class="label">Profile Box</div><textarea class="input" id="profile_box" rows="2"></textarea></div>
<div class="card"><div style="font-weight:900">💰 Money</div><div class="label">Ads Reward ৳</div><input class="input" id="ads_reward" type="number"><div class="label">Ads Limit</div><input class="input" id="ads_limit" type="number"><div class="label">Refer Bonus ৳</div><input class="input" id="refer_bonus" type="number"></div>
</div>

<div id="p2" style="display:none"><div class="card"><div style="font-weight:900">✅ Tasks - Link System (এখানে Link বসাবা)</div><div id="tasksEdit"></div></div></div>

<div id="p3" style="display:none"><div class="card"><div style="font-weight:900">🖼️ Slider 3 টা ছবি</div><div class="label">Image 1 Link</div><input class="input" id="sl1"><div class="label">Image 2 Link</div><input class="input" id="sl2"><div class="label">Image 3 Link</div><input class="input" id="sl3"></div></div>

<button class="btn-save" onclick="saveAll()">💾 SAVE ALL - Live হয়ে যাবে</button>
<script>
let curTasks=[];
...
<b>Task ${i+1}</b><div class="label">Title</div><input class="input" id="tt_${i}" value="${t.title}">
<div class="label">Reward ৳</div><input class="input" id="tr_${i}" type="number" value="${t.reward}">
<div class="label">Link - এখানে তোমার YouTube/Telegram/FB Link বসাও</div><input class="input" id="tl_${i}" value="${t.link}">
...
let tasks=[];
for(let i=0;i<6;i++){
let el=document.getElementById('tt_'+i);if(!el)continue;
tasks.push({title:document.getElementById('tt_'+i).value,reward:parseInt(document.getElementById('tr_'+i).value)||20,link:document.getElementById('tl_'+i).value,color:curTasks[i].color,btn:document.getElementById('tb_'+i).value});
}
let data={
...
fetch('/api/save',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(data)}).then(()=>{alert('✅ Saved - সব Live হয়ে গেছে - এখন App Refresh করো');load();});
}
load();
</script></body></html>
"""
