# -*- coding: utf-8 -*-
# FINAL FIX 1-5 - Gallery Button Fixed - All Buttons Working - A to Z
import os, json
...
def default():
    return {
        ...
        "withdraws":[],
        "settings":{
            "app_name":"Protidiner Kaj BD","admin_name":"SHIBLI NOMAN","app_logo":"👑","admin_profile_img":"","zone":"11764581","bonus":1120,"ad_reward":2,"popup_reward":3,"company_limit":30,"popup_limit":20,"task_limit":5,"min_with":500,
...
<div class="card" style="text-align:center;background:linear-gradient(135deg,#1e293b,#334155)"><div style="font-size:12px;opacity:0.7">ব্যালেন্স</div><div style="font-size:40px;margin:6px 0">৳<span id="bal2" style="font-weight:800">0</span></div><div style="font-size:11px;opacity:0.6">Min ৳<span id="minWith">500</span></div></div>
<div class="card"><h3>💸 Withdraw Method</h3>
<div class="payCard active" id="pay-bkash" onclick="selectPay('bKash')"><div class="payLogo" style="background:#e2136e">৳</div><div><div style="font-weight:800">bKash</div><div style="font-size:11px;opacity:0.6">Personal • Instant</div></div><div style="margin-left:auto;color:#22c55e" id="bkashCheck">✓</div></div>
...
<div class="card"><h3>🚀 দ্রুত যোগাযোগ</h3><div class="payCard" style="border-color:#0ea5e9;background:#0c1a2e" onclick="openTG()"><div class="payLogo" style="background:#0ea5e9">✈️</div><div><div style="font-weight:800">Telegram Support</div></div><div style="margin-left:auto">➡️</div></div><div class="payCard" onclick="openTG()"><div class="payLogo" style="background:#22c55e">💬</div><div><div style="font-weight:800">WhatsApp</div><div style="font-size:11px;opacity:0.6" id="waNum"></div></div><div style="margin-left:auto">➡️</div></div></div>
<div class="card"><h3>❓ FAQ</h3><div class="faq"><b>Q: টাকা কখন পাবো?</b><br><span style="font-size:11px;opacity:0.7">A: 24 ঘণ্টায়</span></div><div class="faq"><b>Q: VPN চলবে?</b><br><span style="font-size:11px;opacity:0.7">A: না</span></div><div class="faq"><b>Q: Refer?</b><br><span style="font-size:11px;opacity:0.7">A: 1 জন = ৳50</span></div></div>
</div>
...
<div style="position:relative;width:100px;margin:0 auto" onclick="openGallery()"><div class="profileBig" id="userBigPic">👤</div><div class="camIcon">📸</div></div>
<div style="margin-top:12px"><div style="font-weight:900;font-size:18px" id="userNameShow">User</div><div style="font-size:11px;opacity:0.6" id="userIdShow"></div><div style="margin-top:6px"><span style="background:#6d4cff;padding:4px 10px;border-radius:20px;font-size:11px;font-weight:700">🥉 Bronze Member</span></div></div>
<div style="margin-top:14px">
...
<button class="btn" style="background:linear-gradient(90deg,#6d4cff,#3a1aff);margin-top:10px" onclick="openGallery()">📸 গ্যালারি থেকে ছবি নিন</button>
<button class="btn" style="background:linear-gradient(90deg,#00c853,#009624);margin-top:8px" onclick="saveUserProfile()">💾 Save Profile</button>
<div style="font-size:10px;opacity:0.5;margin-top:6px">ছবিতে বা বাটনে ক্লিক → গ্যালারি খুলবে → Save দিন</div>
</div>
</div>
<div class="card"><h3>📊 পরিসংখ্যান</h3><div style="display:flex;gap:8px;margin-top:10px"><div class="statBox"><div style="font-size:18px">💰</div><div style="font-weight:800" id="statBal">৳0</div><div style="font-size:10px;opacity:0.6">ব্যালেন্স</div></div><div class="statBox"><div style="font-size:18px">📺</div><div style="font-weight:800" id="statAds">0</div><div style="font-size:10px;opacity:0.6">Ads</div></div><div class="statBox"><div style="font-size:18px">📋</div><div style="font-weight:800" id="statTasks">0</div><div style="font-size:10px;opacity:0.6">Task</div></div></div><div style="display:flex;gap:8px;margin-top:8px"><div class="statBox"><div style="font-size:18px">👥</div><div style="font-weight:800" id="statTotal">0</div><div style="font-size:10px;opacity:0.6">Total</div></div><div class="statBox"><div style="font-size:18px">📅</div><div style="font-weight:800" id="statJoin">-</div><div style="font-size:10px;opacity:0.6">Join</div></div><div class="statBox"><div style="font-size:18px">🆔</div><div style="font-weight:800;font-size:11px" id="statId">-</div><div style="font-size:10px;opacity:0.6">ID</div></div></div></div>
<div class="card"><h3>⚙️ সেটিংস</h3><div class="payCard" onclick="nav('wallet')"><div class="payLogo" style="background:#1e293b">💸</div><div><div style="font-weight:700">Withdraw History</div></div><div style="margin-left:auto">➡️</div></div><div class="payCard" onclick="copyRef()"><div class="payLogo" style="background:#1e293b">🔗</div><div><div style="font-weight:700">My Refer Link</div><div style="font-size:10px;opacity:0.6;word-break:break-all" id="refLink2"></div></div><div style="margin-left:auto">📋</div></div></div>
</div>
...
// FIXED GALLERY - Resize to 300px for mobile
document.getElementById('userPicInput').addEventListener('change',function(e){
 ...
 reader.onload=function(ev){
   ...
   img.onload=function(){
     ...
     document.getElementById('userBigPic').innerHTML=`<img src="${tempUserPic}">`;
...
async function saveUserProfile(){
 ...
 if(!name &&!tempUserPic){alert('নাম বা ছবি দিন');return;}
 let payload={id:uid,name:name};
 if(tempUserPic) payload.profile_img=tempUserPic;
...
<h2>👑 Admin Final 1-5 Fixed</h2>
<div class="card" style="border:1.5px solid #f59e0b"><h3>⭐ ব্যানার</h3><input type="file" id="bannerFile" accept="image/*"><button class="btn" style="background:#f59e0b" onclick="uploadBanner()">📸 যোগ</button><button class="btn" style="background:#ef4444" onclick="clearBanner()">🗑️ মুছুন</button><div id="bannerCount"></div></div>
...
<button class="btn" style="padding:16px;background:#6d4cff" onclick="save()">💾 Save All - সব কাজ করবে</button><div id="msg" style="text-align:center;color:#fde047;margin-top:8px;font-weight:800"></div>
