...
def load_db():
    if not os.path.exists(DB):
        d={"users":{},"withdraws":[],"settings":{"app_name":"Protidiner Kaj BD","admin_name":"SHIBLI NOMAN","slider1":"🎉 Daily Bonus Available Today","slider2":"📢 Company Sponsored - 100% Safe","slider3":"🎁 50 Ads দেখলে ৳100 বোনাস!","slider4":"💰 100% Payment Guaranteed","slider5":"🔥 প্রতিদিন কাজ করুন","company_reward":2,"popup_reward":3,"company_limit":30,"popup_limit":20,"balance_target":2000,"min_withdraw":500,"tele_link":"https://t.me/","yt_link":"https://youtube.com/","fb_link":"https://facebook.com/"}}
        json.dump(d,open(DB,'w',encoding='utf-8'),ensure_ascii=False,indent=2)
        return d
    return json.load(open(DB,'r',encoding='utf-8'))

def save_db(d): json.dump(d,open(DB,'w',encoding='utf-8'),ensure_ascii=False,indent=2)
def get_user(db,uid):
    uid=str(uid)
    if uid not in db["users"]: db["users"][uid]={"id":uid,"name":"User 8385","balance":1120,"company":0,"popup":0,"total":0,"tasks":[],"img":"","join":"2026-09-13"}
    return db["users"][uid]
...
def ag(): return jsonify(load_db())
@app.route('/api/admin/save',methods=['POST'])
def asv(): db=load_db(); db["settings"].update(request.json.get('settings',{})); save_db(db); return jsonify({"msg":"Save"})
@app.route('/api/reward')
def rw():
    db=load_db(); u=get_user(db,request.args.get('id')); t=request.args.get('type'); s=db["settings"]
    if t=='company':
        if u["company"]>=int(s["company_limit"]): return jsonify({"msg":"Limit 30 শেষ"})
        u["company"]+=1; u["balance"]+=int(s["company_reward"])
    else:
        if u["popup"]>=int(s["popup_limit"]): return jsonify({"msg":"Limit 20 শেষ"})
        u["popup"]+=1; u["balance"]+=int(s["popup_reward"])
    u["total"]=u["company"]+u["popup"]; save_db(db); return jsonify({"ok":1})
@app.route('/api/task/done',methods=['POST'])
def td():
    db=load_db(); j=request.json; u=get_user(db,j.get('id')); tid=str(j.get('tid')); amt=int(j.get('amt',20))
    if tid not in u["tasks"]: u["tasks"].append(tid); u["balance"]+=amt
    save_db(db); return jsonify({"msg":f"✅ ৳{amt} Bonus যোগ হয়েছে"})
...
def wd():
    ...
    u["balance"]-=amt; db["withdraws"].append({"uid":u["id"],"amt":amt,"num":j.get('num'),"method":j.get('method'),"status":"Pending","time":str(datetime.now())[:16]}); save_db(db); return jsonify({"msg":"✅ Withdraw পাঠানো হয়েছে"})
...
ADMIN="""<!DOCTYPE html><html><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1.0"><style>body{background:#070710;color:#fff;max-width:500px;margin:0 auto;padding:12px;font-family:system-ui}.card{background:#15152a;border-radius:16px;padding:14px;margin:10px 0}input{width:100%;padding:12px;border-radius:10px;background:#0e0e20;border:1px solid #333;color:#fff;margin-top:5px}label{font-size:11px;color:#aaa;margin-top:8px;display:block}.btn{width:100%;padding:14px;background:#6d4cff;border:none;border-radius:12px;color:#fff;font-weight:900;margin-top:12px}</style></head><body><h2 style="text-align:center">Admin Control</h2><div class="card"><label>Slider1</label><input id="slider1"><label>Slider2</label><input id="slider2"><label>Company Reward</label><input id="company_reward"><label>Popup Reward</label><input id="popup_reward"><label>Balance Target (লাল বার)</label><input id="balance_target"><button class="btn" onclick="saveAll()">SAVE</button></div><script>function load(){fetch('/api/admin/get').then(r=>r.json()).then(d=>{for(let k in d.settings){let e=document.getElementById(k);if(e)e.value=d.settings[k];}});}function saveAll(){let s={};document.querySelectorAll('input').forEach(e=>s[e.id]=e.value);fetch('/api/admin/save',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({settings:s})}).then(r=>r.json()).then(d=>alert('Save'));}load();</script></body></html>"""
...
<div class="balanceCard"><div style="font-size:14px;color:#e0f2fe">💰 আপনার বর্তমান ব্যালেন্স</div><div style="font-size:54px;font-weight:900;margin:6px 0" id="balHome">৳1120</div><div style="display:flex;gap:8px;justify-content:center;margin-top:10px"><span class="pill" id="bC">Company 0/30</span><span class="pill" id="bP">Popup 0/20</span><span class="pill" id="bT">Total 0</span></div><div class="longBar"><div id="progBar" class="longBarFill"></div></div></div>
...
<div class="taskRow"><div style="display:flex;gap:12px;align-items:center"><div>✈️</div><div><b>Telegram Channel Join</b><div style="font-size:12px;color:#9ca3af">৳25</div></div></div><button style="background:#6d4cff;color:#fff;border:none;padding:10px 18px;border-radius:12px;font-weight:800" onclick="doTask('tele',25)">৳25</button></div>
<div class="taskRow"><div style="display:flex;gap:12px;align-items:center"><div>▶️</div><div><b>YouTube Subscribe</b><div style="font-size:12px;color:#9ca3af">৳30</div></div></div><button style="background:#6d4cff;color:#fff;border:none;padding:10px 18px;border-radius:12px;font-weight:800" onclick="doTask('yt',30)">৳30</button></div>
<div class="taskRow"><div style="display:flex;gap:12px;align-items:center"><div>👍</div><div><b>Facebook Page Like</b><div style="font-size:12px;color:#9ca3af">৳20</div></div></div><button style="background:#6d4cff;color:#fff;border:none;padding:10px 18px;border-radius:12px;font-weight:800" onclick="doTask('fb',20)">৳20</button></div>
<div class="taskRow"><div style="display:flex;gap:12px;align-items:center"><div>👥</div><div><b>Refer Friend</b><div style="font-size:12px;color:#9ca3af">৳50</div></div></div><button style="background:#6d4cff;color:#fff;border:none;padding:10px 18px;border-radius:12px;font-weight:800" onclick="goP('profile')">৳50</button></div>
<div class="taskRow"><div style="display:flex;gap:12px;align-items:center"><div>✅</div><div><b>Daily Check-in</b><div style="font-size:12px;color:#9ca3af">৳15</div></div></div><button style="background:#6d4cff;color:#fff;border:none;padding:10px 18px;border-radius:12px;font-weight:800" onclick="doTask('check',15)">৳15</button></div>
</div>
<div class="referBox"><div style="font-size:18px;font-weight:900">🎁 Refer & Earn ৳50</div><div class="referLinkBox" id="refLinkT">https://...</div><button style="background:#fff;color:#4c1d95;border:none;width:100%;padding:12px;border-radius:12px;font-weight:900;margin-top:12px" onclick="copyRefer()">📋 লিংক কপি করুন</button></div>
<div class="teleBox" onclick="window.open('https://t.me/','_blank')"><div><div style="font-size:18px;font-weight:900">📢 Telegram Channel</div><div style="font-size:12px;opacity:.8">আপডেট ও প্রুফ</div></div><button style="background:#fff;color:#0ea5e9;border:none;padding:10px 18px;border-radius:20px;font-weight:800">Join ✈️</button></div>
</div>
...
<div id="bkCard" class="methodCard active" onclick="selPay('bKash')"><div style="width:52px;height:52px;background:#e2136e;border-radius:14px;display:flex;align-items:center;justify-content:center;font-weight:900">৳</div><div><b>bKash</b><div style="font-size:12px;color:#9ca3af">Personal • Instant Payment</div></div><div style="margin-left:auto;color:#22c55e" id="bkCheck">✓</div></div>
<div id="ngCard" class="methodCard" onclick="selPay('Nagad')"><div style="width:52px;height:52px;background:#ff8c00;border-radius:14px;display:flex;align-items:center;justify-content:center;font-weight:900">৳</div><div><b>Nagad</b><div style="font-size:12px;color:#9ca3af">Personal • Fast Withdraw</div></div><div style="margin-left:auto;color:#22c55e;display:none" id="ngCheck">✓</div></div>
<input id="accNum" class="inputField" placeholder="01XXXXXXXXXX - Personal Number"><input id="wdAmt" class="inputField" type="number" placeholder="500 - Min Withdraw"><button class="withdrawBtn" onclick="doWd()">🚀 Withdraw করুন</button></div>
</div>

<div id="p-support" class="page">
<div class="supportTopBox"><div style="font-size:22px;font-weight:900">আমরা আছি আপনার পাশে</div><div style="font-size:13px;margin-top:6px;opacity:.85">২৪ ঘণ্টা সাপোর্ট • 100% Trusted • SHIBLI NOMAN Team</div></div>
<div class="contactBox">
<div class="contactRow" onclick="window.open('https://t.me/','_blank')"><div style="width:52px;height:52px;border-radius:14px;background:#0ea5e9;display:flex;align-items:center;justify-content:center">✈️</div><div><div style="font-weight:700">Telegram Support (Fast Reply)</div><div style="font-size:12px;color:#9ca3af">2 মিনিটে রিপ্লাই</div></div><div style="margin-left:auto">➡️</div></div>
...
<script>
let uid='8807178385', method='bKash', cur=0;
function goP(p){
...
function init(){
 fetch('/api/get?id='+uid).then(r=>r.json()).then(d=>{
  ...
  for(let i=1;i<=5;i++){ let el=document.getElementById('st'+i); if(el) el.innerText=s['slider'+i]; }
  document.getElementById('balHome').innerText='৳'+u.balance;
  document.getElementById('wBal').innerText='৳'+u.balance;
  ...
  document.getElementById('sBal').innerText='৳'+u.balance;
  document.getElementById('sAds').innerText=u.total;
  document.getElementById('pNameTop').innerText=u.name;
  document.getElementById('refLinkT').innerText='https://telegram-bot-1-v77g.onrender.com/?ref='+u.id;
  let pct=Math.min(100,Math.round((u.balance/parseInt(s.balance_target||2000))*100));
  document.getElementById('progBar').style.width=pct+'%';
 });
}
function doR(t){
 fetch('/api/reward?id='+uid+'&type='+t).then(r=>r.json()).then(d=>{
  if(d.msg && d.msg.includes('Limit')){ alert(d.msg); return; }
  init();
  if(typeof show_11764581==='function') show_11764581();
 });
}
function doTask(id,amt){
 if(id=='tele') window.open('https://t.me/','_blank');
 if(id=='yt') window.open('https://youtube.com/','_blank');
 if(id=='fb') window.open('https://facebook.com/','_blank');
 fetch('/api/task/done',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({id:uid,tid:id,amt:amt})}).then(r=>r.json()).then(d=>{alert(d.msg); init();});
}
function selPay(m){
 method=m;
...
function doWd(){
 let num=document.getElementById('accNum').value;
 let amt=document.getElementById('wdAmt').value;
 if(!num ||!amt){ alert('নাম্বার ও টাকা দিন'); return; }
 fetch('/api/withdraw',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({id:uid,num:num,amt:amt,method:method})}).then(r=>r.json()).then(d=>{alert(d.msg); init();});
}
function copyRefer(){
 let t=document.getElementById('refLinkT').innerText;
 navigator.clipboard.writeText(t).then(()=>alert('✅ Refer Link কপি হয়েছে: '+t));
}
setInterval(()=>{ cur=(cur+1)%5; for(let i=1;i<=5;i++){ let s=document.getElementById('s'+i); let d=document.getElementById('d'+i); if(s) s.classList.toggle('active',i-1==cur); if(d) d.classList.toggle('active',i-1==cur); }},2500);
init();
</script></body></html>
"""
