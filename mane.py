# -*- coding: utf-8 -*-
# FINAL mane.py - একটা ফাইলেই সব - আগের ডিজাইন কাটবে না + কালার সিস্টেম + 2 Ads + No Logout
import os, json
...
def load_db():
    if not os.path.exists(DB):
        data={"users":{},"withdraws":[],"settings":{
            ...
            "official_banners":[],"google_ads":["🎉 Daily Bonus Available","⭐ Official Ad • bKash • Nagad","📢 Company Sponsored"],
        ...
        },"tasks":[{"id":1,"title":"Join Telegram Channel","reward":10,"link":"https://t.me/","type":"telegram"},{"id":2,"title":"Subscribe YouTube","reward":15,"link":"https://youtube.com/","type":"youtube"},{"id":3,"title":"Follow Facebook","reward":10,"link":"https://facebook.com/","type":"facebook"}]}
...
def ads_complete():
    ...
    if ref and ref in db["users"] and u["ads_watched"]>=s["refer_condition_ads"] and u["refer_status"]=="pending":
        db["users"][ref]["balance"]+=s["refer_reward"]; u["refer_status"]="completed"
    save_db(db); return jsonify({"msg":msg,"balance":u["balance"]})
...
def user_update():
    ...
    save_db(db); return jsonify({"msg":"✅ প্রোফাইল সেভ","user":u})

@app.route('/api/admin/save',methods=['POST'])
def api_save():
    db=load_db(); j=request.json
    for k in j: db["settings"][k]=j[k]
    save_db(db); return jsonify({"msg":"✅ Saved - সব সেভ হয়েছে"})
...
body{background:{{bg_color}};color:{{text_color}};max-width:430px;margin:0 auto;padding-bottom:125px}
...
.btn2{background:linear-gradient(90deg,{{btn2_color}},#ef4444)}
.profile{width:52px;height:52px;border-radius:50%;background:linear-gradient(135deg,#1e293b,#334155);display:flex;align-items:center;justify-content:center;border:2px solid {{btn_color}};overflow:hidden;font-size:26px;cursor:pointer}
...
<div class="card"><h4 id="balTitle">{{balance_title}}</h4><h1 id="bal2" style="margin-top:8px;color:#22c55e">৳0</h1><p style="opacity:.6;font-size:12px;margin-top:4px">Company: <span id="cLim">0/30</span> | Popup: <span id="pLim">0/20</span></p></div>
...
<div class="card"><h4>👥 রেফার</h4><p style="font-size:12px;opacity:.7">বন্ধু 10 টা Ads দেখলে ৳50 পাবেন</p><input id="refLink" style="width:100%;padding:10px;border-radius:8px;border:1px solid #333;background:#0a0a1a;color:#fff;margin-top:6px" readonly><button class="btn" onclick="copyRef()">🔗 কপি</button></div>
</div>
<div id="profile" class="page">
<div class="card"><h4>প্রোফাইল</h4><input id="editName" placeholder="নাম" style="width:100%;padding:12px;border-radius:10px;border:1px solid #333;background:#0a0a1a;color:#fff;margin-top:10px"><input type="file" id="imgFile" accept="image/*" style="margin-top:10px"><button class="btn" onclick="saveProfile()">💾 সেভ</button></div>
<div class="card"><h4>💸 Withdraw</h4><select id="wMethod" style="width:100%;padding:12px;border-radius:10px;background:#0a0a1a;color:#fff;border:1px solid #333"><option>bKash</option><option>Nagad</option></select><input id="wNumber" placeholder="01XXXXXXXXX" style="width:100%;padding:12px;border-radius:10px;border:1px solid #333;background:#0a0a1a;color:#fff;margin-top:8px"><input id="wAmount" type="number" placeholder="Amount" style="width:100%;padding:12px;border-radius:10px;border:1px solid #333;background:#0a0a1a;color:#fff;margin-top:8px"><button class="btn" onclick="doWithdraw()">Withdraw</button></div>
</div>
...
document.getElementById('refLink').value=window.location.origin+"/?ref="+d.user.id; document.getElementById('editName').value=d.user.name;
...
def admin():
    db=load_db(); s=db["settings"]
    admin_html = """
<!DOCTYPE html><html><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>Admin Everything</title>
<style>*{box-sizing:border-box;font-family:system-ui}body{max-width:600px;margin:0 auto;padding:12px;background:#070710;color:#fff}.card{padding:14px;border:1px solid #222;border-radius:12px;margin-top:12px;background:#111128}input{width:100%;padding:10px;border-radius:8px;border:1px solid #333;background:#0a0a1a;color:#fff;margin-top:6px}.btn{width:100%;padding:12px;border:none;border-radius:10px;font-weight:800;color:#fff;background:#6d4cff;margin-top:10px;cursor:pointer}label{font-size:12px;opacity:.8}</style></head><body>
<h2>👑 Admin - Everything + Color</h2>
<div class="card"><h4>💰 টাকার বাটন</h4>
<input id="bonus" placeholder="Join Bonus"><input id="ad_reward" placeholder="Company Reward"><input id="popup_reward" placeholder="Popup Reward"><input id="refer_reward" placeholder="Refer Reward"><input id="refer_condition_ads" placeholder="Refer Condition Ads"><input id="company_limit" placeholder="Company Limit"><input id="popup_limit" placeholder="Popup Limit"><input id="min_with" placeholder="Min Withdraw"></div>
<div class="card"><h4>🎨 রং পরিবর্তন - যেকোনো রং Admin থেকে</h4>
<div style="display:flex;gap:8px;align-items:center"><input type="color" id="bg_color" style="width:60px;height:40px"><label>Background #070710</label></div>
<div style="display:flex;gap:8px;align-items:center"><input type="color" id="card_color" style="width:60px;height:40px"><label>Card</label></div>
<div style="display:flex;gap:8px;align-items:center"><input type="color" id="top_color" style="width:60px;height:40px"><label>Top Bar</label></div>
<div style="display:flex;gap:8px;align-items:center"><input type="color" id="btn_color" style="width:60px;height:40px"><label>Button 1</label></div>
<div style="display:flex;gap:8px;align-items:center"><input type="color" id="btn2_color" style="width:60px;height:40px"><label>Button 2</label></div>
<div style="display:flex;gap:8px;align-items:center"><input type="color" id="text_color" style="width:60px;height:40px"><label>Text</label></div>
</div>
<div class="card"><h4>📝 লেখা + লিংক</h4><input id="offer_title" placeholder="Offer Title"><input id="offer_desc" placeholder="Offer Desc"><input id="balance_title" placeholder="Balance Title"><input id="direct_link" placeholder="Direct Link"><input id="zone" placeholder="Zone"><input id="app_name" placeholder="App Name">
<button class="btn" onclick="save()">💾 Save All - রং সহ</button></div>
...
}); fetch('/api/admin/users').then(r=>r.json()).then(u=>{ let h=""; for(let id in u){ let x=u[id]; h+=`<div style="border-bottom:1px solid #222;padding:8px 0"><b>${x.name}</b> (${x.id})<br>৳${x.balance} | Ads:${x.ads_watched} | Join:${x.join_time}<br>Ref By:${x.referred_by||'None'} | Refer:${x.refer_list.length} জন | ${x.refer_status}</div>`; } document.getElementById('users').innerHTML=h; });
...
function save(){ let data={}; document.querySelectorAll('input').forEach(i=>{ if(i.value) data[i.id]= isNaN(i.value) || i.type=='color'? i.value : (i.value.includes('#')? i.value : parseInt(i.value)); if(i.type=='color') data[i.id]=i.value; }); fetch('/api/admin/save',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(data)}).then(r=>r.json()).then(d=>{ alert(d.msg); location.reload(); }); }
load();
</script></body></html>
    """
