# -*- coding: utf-8 -*-
# FINAL A to Z - Protidiner Kaj BD - One File
import os, json
...
def api_save():
    ...
    for k in j:
        if k.startswith("google_ad"):
            ...
            db["settings"][k] = j[k]
    save(db)
    return jsonify({"msg": "✅ Saved - অ্যাপে সাথে সাথে আপডেট"})
...
def upload():
    ...
    if 'img' in j:
        ...
        save(db)
        return jsonify({"msg": "✅ প্রোফাইল ছবি আপডেট"})
    if 'banner' in j:
    ...
    if 'clear_banner' in j:
        ...
        save(db)
        return jsonify({"msg": "🗑️ সব ব্যানার মুছা হয়েছে"})
...
.bannerBox{margin:12px;border-radius:20px;overflow:hidden;height:180px;background:linear-gradient(90deg,#f59e0b,#ef4444);position:relative;border:2px solid rgba(255,255,255,0.15)}
.bannerBox img{width:100%;height:100%;object-fit:cover}
.bannerText{position:absolute;bottom:0;left:0;right:0;background:linear-gradient(transparent,rgba(0,0,0,0.85));padding:32px 14px 12px;font-weight:800;text-align:center;font-size:14px}
...
<div class="bannerBox" id="bannerBox"><div style="height:100%;display:flex;align-items:center;justify-content:center;padding:20px;text-align:center;font-weight:900" id="gAd">Loading...</div></div>

<div class="card" style="text-align:center;background:linear-gradient(135deg,#1e3a8a,#3b82f6,#06b6d4,#10b981,#f59e0b);padding:30px 18px;border:2px solid rgba(255,255,255,0.25);box-shadow:0 10px 40px rgba(59,130,246,0.35)">
<div style="font-size:11px;letter-spacing:2px;color:#e0f2fe;font-weight:700" id="balTitle">💰 আপনার বর্তমান ব্যালেন্স</div>
<div style="font-size:58px;font-weight:900;margin:10px 0;color:#fff;text-shadow:0 2px 20px rgba(0,0,0,0.6)">৳<span id="bal">0</span></div>
<div style="display:flex;justify-content:center;gap:6px;flex-wrap:wrap">
<span style="background:rgba(0,0,0,0.35);padding:6px 10px;border-radius:20px;font-size:11px;font-weight:700">Company <b id="ads" style="color:#fde047">0</b>/<span id="adsLim">30</span></span>
<span style="background:rgba(0,0,0,0.35);padding:6px 10px;border-radius:20px;font-size:11px;font-weight:700">Popup <b id="pop" style="color:#86efac">0</b>/<span id="popLim">20</span></span>
<span style="background:rgba(0,0,0,0.35);padding:6px 10px;border-radius:20px;font-size:11px;font-weight:700">Total <b id="total">0</b></span>
</div>
<div style="background:rgba(0,0,0,0.4);height:10px;border-radius:20px;margin-top:16px;overflow:hidden;border:1px solid rgba(255,255,255,0.2)"><div id="prog" style="height:100%;background:linear-gradient(90deg,#fde047,#fbbf24);width:0%;border-radius:20px;transition:width 1s"></div></div>
</div>
...
<button class="btn" style="background:#1e293b" onclick="alert('Task Page - 5 টা/দিন')">📋 TASK BONUS - 5 টা/দিন</button>
</div>
...
<div class="btm">
<div class="on"><span>🏠</span>Home</div><div><span>📋</span>Task</div><div><span>💰</span>Wallet</div><div><span>💬</span>Support</div><div><span>👤</span>Profile</div>
</div>
...
async function load(){
 ...
 document.getElementById('bal').innerText=u.balance;document.getElementById('ads').innerText=u.ads_today;document.getElementById('pop').innerText=u.popup_today;
 document.getElementById('ads2').innerText=u.ads_today;document.getElementById('pop2').innerText=u.popup_today;document.getElementById('total').innerText=u.total;
 ...
 if(s.admin_profile_img && s.admin_profile_img.startsWith('data:image')){pb.innerHTML=`<img src="${s.admin_profile_img}">`;}else{pb.innerHTML=s.admin_profile||'👤';}
 renderBanner();
}
function renderBanner(){
 ...
 if(banners.length>0){
   ...
   box.innerHTML=`<img src="${img}"><div class="bannerText">${gAds[idx % gAds.length]||''}</div>`;
 }else{
   box.innerHTML=`<div style="height:100%;display:flex;align-items:center;justify-content:center;padding:20px;text-align:center;font-weight:900;font-size:15px;background:linear-gradient(90deg,#f59e0b,#ef4444)">${gAds[idx % gAds.length]||'Official Ad'}</div>`;
 }
}
...
.prev2{width:70px;height:70px;border-radius:50%;object-fit:cover;margin-top:8px;border:2px solid #6d4cff;display:none}
</style>
</head><body>
<h2>👑 Full Admin Panel - A to Z Control</h2>

<div class="card" style="border:2px solid #f59e0b">
<h3>⭐ উপরের Official Banner - বড় বক্স (ফটো)</h3>
<p style="font-size:12px;opacity:0.7">bKash, Nagad, Offer Photo এখানে দাও - 5 টা পর্যন্ত স্লাইড হবে</p>
<input type="file" id="bannerFile" accept="image/*">
<img id="bannerPrev" class="prev">
<button class="btn" style="background:#f59e0b" onclick="uploadBanner()">📸 ব্যানার যোগ করুন</button>
...
<h3>👤 উপরের প্রোফাইল ছবি - বড় (52px)</h3>
<p style="font-size:12px;opacity:0.7">টিপ দিলে গ্যালারি খুলবে</p>
<input type="file" id="fileIn" accept="image/*">
<img id="preview" class="prev2">
<button class="btn" style="background:#00c853" onclick="uploadImg()">📤 প্রোফাইল আপলোড</button>
</div>

<div class="card"><h3>📝 নাম ও লোগো</h3>App Name:<input id="app_name">Admin Name:<input id="admin_name">Logo Emoji:<input id="app_logo" placeholder="👑"></div>
<div class="card"><h3>📢 Google Ad লেখা (3 টা)</h3>Ad 1:<input id="g1">Ad 2:<input id="g2">Ad 3:<input id="g3"></div>
...
<button class="btn" style="background:#6d4cff;padding:18px;font-size:16px" onclick="save()">💾 সব Save - সাথে সাথে অ্যাপে আপডেট</button>
<div id="msg" style="text-align:center;margin-top:12px;font-weight:700"></div>
...
async function uploadBanner(){if(!banner64){alert('ব্যানার ফটো সিলেক্ট করো');return;}let res=await fetch('/api/admin/upload',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({banner:banner64})}).then(x=>x.json());msg.innerText=res.msg;load();banner64='';bannerPrev.style.display='none';}
...
async function load(){
 let s=await fetch('/api/get?id=8807178385').then(x=>x.json()).then(x=>x.settings);
 app_name.value=s.app_name;admin_name.value=s.admin_name;app_logo.value=s.app_logo;
 g1.value=s.google_ads[0];g2.value=s.google_ads[1];g3.value=s.google_ads[2];
 offer_title.value=s.offer_title;offer_desc.value=s.offer_desc;
 company_limit.value=s.company_limit;ad_reward.value=s.ad_reward;popup_limit.value=s.popup_limit;popup_reward.value=s.popup_reward;
 bannerCount.innerText=`বর্তমানে ${s.official_banners.length}/5 টা ব্যানার আছে`;
 if(s.admin_profile_img){preview.src=s.admin_profile_img;preview.style.display='block';base64=s.admin_profile_img;}
}
async function save(){
 let d={app_name:app_name.value,admin_name:admin_name.value,app_logo:app_logo.value,google_ad1:g1.value,google_ad2:g2.value,google_ad3:g3.value,offer_title:offer_title.value,offer_desc:offer_desc.value,company_limit:parseInt(company_limit.value),ad_reward:parseInt(ad_reward.value),popup_limit:parseInt(popup_limit.value),popup_reward:parseInt(popup_reward.value)};
