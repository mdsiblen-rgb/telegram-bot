# -*- coding: utf-8 -*-
# FINAL A-Z FULL - Page 1 + 2 + 3 - No Cut - Everything Included
import os, json
...
def default():
    return {
        ...
        "settings":{
            ...
            "min_with":500,
            "official_banners":[],
            "google_ads":["🎉 Daily Bonus Available Today","⭐ Official Ad • bKash • Nagad • Daraz • Trusted","📢 Company Sponsored • 100% Safe"],
...
def api_reward():
    ...
    if typ=='company':
        if u["ads_today"]>=s["company_limit"]: return jsonify({"msg":f"Company Limit {s['company_limit']} শেষ"})
        u["ads_today"]+=1; u["balance"]+=s["ad_reward"]
    else:
        if u["popup_today"]>=s["popup_limit"]: return jsonify({"msg":f"Popup Limit {s['popup_limit']} শেষ"})
...
def task_complete():
    ...
    if len(u["tasks_done"])>=s["task_limit"]: return jsonify({"msg":f"আজকে {s['task_limit']} টা লিমিট শেষ"})
...
def withdraw():
    ...
    if amt < s["min_with"]: return jsonify({"msg":f"মিনিমাম {s['min_with']} টাকা লাগবে"})
    if u["balance"] < amt: return jsonify({"msg":"ব্যালেন্স কম আছে"})
    ...
    save(db); return jsonify({"msg":f"✅ {method} ৳{amt} Withdraw Request সফল"})
...
def upload():
    db=load(); j=request.json
    if 'img' in j: db["settings"]["admin_profile_img"]=j['img']; save(db); return jsonify({"msg":"✅ প্রোফাইল ছবি আপডেট হয়েছে"})
    if 'banner' in j:
        if len(db["settings"]["official_banners"])>=5: db["settings"]["official_banners"].pop(0)
        db["settings"]["official_banners"].append(j['banner']); save(db); return jsonify({"msg":f"✅ ব্যানার {len(db['settings']['official_banners'])}/5 যোগ"})
    if 'clear_banner' in j: db["settings"]["official_banners"]=[]; save(db); return jsonify({"msg":"🗑️ সব ব্যানার মুছা হয়েছে"})
...
.bannerBox{margin:12px;border-radius:22px;overflow:hidden;height:165px;background:linear-gradient(90deg,#f59e0b,#ef4444);position:relative;border:2px solid rgba(255,255,255,0.15);animation:blinkGlow 2.2s infinite ease-in-out}
...
<div class="top"><div style="display:flex;align-items:center;gap:10px"><div style="font-size:28px" id="appLogo">👑</div><div><div style="font-weight:900;font-size:16px;display:flex;align-items:center;gap:6px"><span id="appName">Protidiner Kaj BD</span><span style="background:#22c55e;color:#fff;font-size:10px;padding:2px 6px;border-radius:6px">✓</span></div><div style="font-size:11px;opacity:0.6" id="adminName">Admin: SHIBLI NOMAN</div></div></div><div class="profile" id="profBox">👤</div></div>
...
<!-- ================= PAGE 2 - TASK - FULL + BOTTOM 3 BOXES ================= -->
...
<!-- নিচের 3 বক্স - ফাঁকা জায়গা ভরাট -->
<div style="margin-top:18px;background:linear-gradient(135deg,#6d4cff,#3a1aff);border-radius:16px;padding:16px;position:relative;overflow:hidden"><div class="shine"></div><div style="position:relative;z-index:3"><div style="font-weight:900;font-size:15px">🎁 Refer & Earn - ৳50 প্রতি রেফারে</div><div style="font-size:12px;margin-top:4px;opacity:0.9">বন্ধুকে শেয়ার করো, ও জয়েন করলেই ৳50</div><div style="background:rgba(0,0,0,0.35);border-radius:10px;padding:10px;margin-top:10px;font-size:11px;word-break:break-all" id="refLink"></div><button class="btn" style="background:#fff;color:#3a1aff;margin-top:10px" onclick="copyRef()">📋 লিংক কপি করুন</button></div></div>
<div style="margin-top:12px;background:linear-gradient(135deg,#0ea5e9,#0284c7);border-radius:16px;padding:16px;display:flex;justify-content:space-between;align-items:center"><div><div style="font-weight:800">📢 আমাদের Telegram Channel</div><div style="font-size:11px;opacity:0.9">সকল আপডেট ও পেমেন্ট প্রুফ</div></div><button class="btn" style="width:auto;background:#fff;color:#0284c7;padding:10px 16px;margin:0" onclick="openTG()">Join ✈️</button></div>
<div style="margin-top:12px;background:#1a1a2e;border:1px solid #2a2a4a;border-radius:16px;padding:14px"><div style="font-weight:800;font-size:13px">⚠️ Task নিয়মাবলী</div><div style="font-size:11px;opacity:0.7;margin-top:6px;line-height:1.6">• প্রতিদিন 5 টা Task করতে পারবেন<br>• লিংকে গিয়ে জয়েন/সাবস্ক্রাইব করুন<br>• Verify করলে সাথে সাথে টাকা যোগ<br>• ভুল করলে Task বাতিল হবে</div></div>
</div>
</div>

<!-- ================= PAGE 3 - WALLET - FULL + LOGO + BOTTOM BOX ================= -->
<div id="p-wallet" class="page">
<div class="card" style="text-align:center;background:linear-gradient(135deg,#1e293b,#334155);border:1px solid #475569"><div style="font-size:12px;opacity:0.7">আপনার বর্তমান ব্যালেন্স</div><div style="font-size:40px;font-weight:600;margin:6px 0">৳<span id="bal2" style="font-weight:800">0</span></div><div style="font-size:11px;opacity:0.6">Min Withdraw ৳<span id="minWith">500</span></div></div>
<div class="card"><h3 style="font-size:15px">💸 Withdraw Method - সিলেক্ট করুন</h3>
<div class="payCard active" id="pay-bkash" onclick="selectPay('bKash')"><div class="payLogo" style="background:#e2136e">bKash</div><div><div style="font-weight:800">bKash</div><div style="font-size:11px;opacity:0.6">Personal • Instant Payment</div></div><div style="margin-left:auto;color:#22c55e;font-weight:800" id="bkashCheck">✓</div></div>
<div class="payCard" id="pay-nagad" onclick="selectPay('Nagad')"><div class="payLogo" style="background:#f69220">Nagad</div><div><div style="font-weight:800">Nagad</div><div style="font-size:11px;opacity:0.6">Personal • Fast Withdraw</div></div><div style="margin-left:auto;color:#6b7280" id="nagadCheck">○</div></div>
<div style="margin-top:14px"><label style="font-size:12px;opacity:0.7">আপনার নাম্বার</label><input id="withNumber" type="tel" placeholder="01XXXXXXXXX" style="width:100%;padding:14px;margin-top:6px;border-radius:12px;border:1px solid #2a2a4a;background:#15152a;color:#fff"><label style="font-size:12px;opacity:0.7;margin-top:10px;display:block">পরিমাণ (৳)</label><input id="withAmount" type="number" placeholder="500" style="width:100%;padding:14px;margin-top:6px;border-radius:12px;border:1px solid #2a2a4a;background:#15152a;color:#fff"><button class="btn" style="background:linear-gradient(90deg,#e2136e,#f69220);padding:16px;margin-top:14px;font-size:15px" onclick="doWithdraw()">🚀 Withdraw করুন</button></div>
</div>
<div class="card" style="background:linear-gradient(135deg,#064e3b,#065f46);border:1px solid #10b981"><div style="font-weight:800;font-size:14px">✅ Withdraw নিয়ম - নিচের বক্স</div><div style="font-size:12px;opacity:0.85;margin-top:8px;line-height:1.7">- মিনিমাম ৳500 হলে Withdraw<br>- bKash / Nagad Personal নাম্বার দিন<br>- 24 ঘণ্টার মধ্যে পেমেন্ট পাবেন<br>- ভুল নাম্বার দিলে টাকা হারাবে<br>- প্রতিদিন 1 বার Withdraw করা যাবে</div></div>
<div class="card"><h3 style="font-size:14px">📜 Withdraw History</h3><div id="withHistory" style="margin-top:10px;font-size:12px;opacity:0.7">কোনো Withdraw নেই</div></div>
</div>
...
<div id="p-profile" class="page"><div class="card"><h3>👤 Profile</h3><p>ID: <span id="uid"></span><br>Balance: ৳<span id="bal3">0</span><br>Total Task: <span id="total2">0</span></p></div></div>
...
function selectPay(m){selectedPay=m;document.getElementById('pay-bkash').classList.toggle('active',m=='bKash');document.getElementById('pay-nagad').classList.toggle('active',m=='Nagad');document.getElementById('bkashCheck').innerText=m=='bKash'?'✓':'○';document.getElementById('nagadCheck').innerText=m=='Nagad'?'✓':'○';document.getElementById('bkashCheck').style.color=m=='bKash'?'#22c55e':'#6b7280';document.getElementById('nagadCheck').style.color=m=='Nagad'?'#22c55e':'#6b7280';}
async function load(){
...
function copyRef(){navigator.clipboard.writeText(document.getElementById('refLink').innerText);alert('✅ Refer Link Copy হয়েছে');}
...
<div class="card" style="border:1.5px solid #f59e0b"><h3>⭐ বড় ব্যানার - ফটো (মিট মিট করবে)</h3><input type="file" id="bannerFile" accept="image/*"><button class="btn" style="background:#f59e0b" onclick="uploadBanner()">📸 ব্যানার যোগ (5 টা)</button><button class="btn" style="background:#ef4444" onclick="clearBanner()">🗑️ সব মুছুন</button><div id="bannerCount"></div></div>
<div class="card" style="border:1.5px solid #6d4cff"><h3>👤 প্রোফাইল ছবি (52px বড় - গ্যালারি থেকে)</h3><input type="file" id="fileIn" accept="image/*"><button class="btn" style="background:#00c853" onclick="uploadImg()">📤 প্রোফাইল আপলোড</button></div>
<div class="card"><h3>📝 নাম / লোগো / লিংক</h3>App Name:<input id="app_name">Admin Name:<input id="admin_name">Logo:<input id="app_logo">TG Channel:<input id="tg_channel">Support:<input id="support_link">Balance Title:<input id="balance_title"></div>
<div class="card"><h3>📢 উপরের লেখা (ঘুরবে)</h3>Ad 1:<input id="g1">Ad 2:<input id="g2">Ad 3:<input id="g3"></div>
<div class="card" style="border:1.5px solid #00c853"><h3>📋 Task Page - 5 টা (এডমিন থেকে চেঞ্জ)</h3><div id="taskAdmin"></div></div>
<div class="card"><h3>🎁 অফার বক্স</h3>Title:<input id="offer_title">Desc:<textarea id="offer_desc"></textarea></div>
<div class="card"><h3>💰 লিমিট ও টাকা</h3>Company Limit:<input id="company_limit" type="number">Reward:<input id="ad_reward" type="number">Popup Limit:<input id="popup_limit" type="number">Reward:<input id="popup_reward" type="number">Task Limit:<input id="task_limit" type="number">Min Withdraw:<input id="min_with" type="number"></div>
<button class="btn" style="padding:16px;font-size:15px;background:#6d4cff" onclick="save()">💾 Save All - সাথে সাথে অ্যাপে আপডেট</button><div id="msg" style="text-align:center;color:#fde047;margin-top:8px;font-weight:800"></div>
<div class="card"><h3>💸 Withdraw Requests - সব</h3><div id="wdList" style="font-size:12px"></div></div>
<script>
let base64="",banner64="";
...
async function load(){let data=await fetch('/api/get?id=8807178385').then(x=>x.json());let s=data.settings;app_name.value=s.app_name;admin_name.value=s.admin_name;app_logo.value=s.app_logo;tg_channel.value=s.tg_channel||'';support_link.value=s.support_link||'';balance_title.value=s.balance_title;g1.value=s.google_ads[0];g2.value=s.google_ads[1];g3.value=s.google_ads[2];offer_title.value=s.offer_title;offer_desc.value=s.offer_desc;company_limit.value=s.company_limit;ad_reward.value=s.ad_reward;popup_limit.value=s.popup_limit;popup_reward.value=s.popup_reward;task_limit.value=s.task_limit;min_with.value=s.min_with;bannerCount.innerText=`ব্যানার ${s.official_banners.length}/5 - মিট মিট করবে`;let ta=document.getElementById('taskAdmin');ta.innerHTML='';data.tasks.forEach(t=>{ta.innerHTML+=`<div style="border:1px solid #333;padding:8px;border-radius:8px;margin:6px 0;background:#151515"><b>Task ${t.id}</b><br>Title:<input id="task_${t.id}_title" value="${t.title}"> Icon:<input id="task_${t.id}_icon" value="${t.icon}"> Reward:<input id="task_${t.id}_reward" type="number" value="${t.reward}"> Link:<input id="task_${t.id}_link" value="${t.link||''}"> Desc:<input id="task_${t.id}_desc" value="${t.desc||''}"></div>`;});let wd=document.getElementById('wdList');wd.innerHTML='';(data.all_withdraws||[]).slice(-30).reverse().forEach(w=>{wd.innerHTML+=`<div style="border:1px solid #333;padding:8px;margin:4px 0;border-radius:6px">${w.uid} | ${w.method} ৳${w.amount} | ${w.number} | ${w.status} | ${w.time}</div>`;});}
