...
def load_db():
    if not os.path.exists(DB):
        d={"users":{},"withdraws":[],"settings":{
            ...
            "slider_txt1":"1 - Daily Bonus Available Today","slider_txt2":"2 - Company Sponsored 100% Safe","slider_txt3":"3 - 50 Ads দেখলে 100TK Bonus","slider_txt4":"4 - 100% Payment Guaranteed","slider_txt5":"5 - প্রতিদিন কাজ করুন",
...
def taskdone():
    ...
    rw=20; u["tasks"].append(tid); u["balance"]+=rw; u["total"]+=1; save_db(db); return jsonify({"msg":f"✅ {rw} TK"})
...
def asave():
    ...
    save_db(db); return jsonify({"msg":"✅ বড় ফাইল SAVE - 1 থেকে 5 সব সেভ হয়েছে"})
...
<div class="top"><div style="display:flex;gap:10px;align-items:center"><div style="font-size:32px">👑</div><div><div style="font-weight:800">Protidiner Kaj BD ✓</div><div style="font-size:12px;color:#9ca3af">Admin: SHIBLI NOMAN</div></div></div><div onclick="goPage('profile')" style="width:46px;height:46px;border-radius:50%;border:2px solid #6d4cff;background:#1e293b;display:flex;align-items:center;justify-content:center;overflow:hidden"><img id="topAv" src="" style="display:none;width:100%;height:100%;object-fit:cover"><span id="topAvT">👤</span></div></div>

<div id="p-home" class="page active">
<div class="card halkaDew" style="padding:0;border:none;overflow:hidden"><div class="slider" id="mainSlider"><div class="slide active" id="s1" style="background:linear-gradient(90deg,#f59e0b,#ef4444)"><span id="st1">1 - Daily Bonus</span></div><div class="slide" id="s2" style="background:linear-gradient(90deg,#06b6d4,#3b82f6)"><span id="st2">2 - Company Safe</span></div><div class="slide" id="s3" style="background:linear-gradient(90deg,#10b981,#06b6d4)"><span id="st3">3 - Bonus</span></div><div class="slide" id="s4" style="background:linear-gradient(90deg,#8b5cf6,#ec4899)"><span id="st4">4 - Payment Guaranteed</span></div><div class="slide" id="s5" style="background:linear-gradient(90deg,#f97316,#eab308)"><span id="st5">5 - প্রতিদিন কাজ</span></div></div><div class="dotWrap"><div class="dot active" id="d1"></div><div class="dot" id="d2"></div><div class="dot" id="d3"></div><div class="dot" id="d4"></div><div class="dot" id="d5"></div></div></div>
<div class="card halkaDew" style="background:linear-gradient(135deg,#1d4ed8,#06b6d4,#10b981);text-align:center;border:none"><div style="font-size:13px">ব্যালেন্স</div><div style="font-size:56px;font-weight:900" id="balMain">৳1120</div><div style="display:flex;gap:8px;justify-content:center"><span style="background:#0005;padding:6px 12px;border-radius:20px;font-size:12px" id="c1">Company 0/30</span><span style="background:#0005;padding:6px 12px;border-radius:20px;font-size:12px" id="c2">Popup 0/20</span><span style="background:#0005;padding:6px 12px;border-radius:20px;font-size:12px" id="c3">Total 0</span></div><div class="progressWrap"><div id="progressBar" class="progressBar"></div></div></div>
<div class="card"><button class="btn" style="background:#7c3aed" onclick="doCompany()">📺 COMPANY ADS <span id="btnC">0/30</span></button><button class="btn" style="background:#16a34a" onclick="doPopup()">💰 POPUP ADS <span id="btnP">0/20</span></button><button class="btn" style="background:#1e293b" onclick="goPage('tasks')">📋 TASK BONUS - 5 টা</button></div>
</div>

<div id="p-tasks" class="page"><div class="card"><div style="font-weight:800;font-size:18px;margin-bottom:10px">Task Bonus 1-5</div><div id="taskList"></div></div></div>
<div id="p-wallet" class="page"><div class="card halkaDew" style="background:#1e293b;text-align:center"><div>ব্যালেন্স</div><div style="font-size:52px;font-weight:900" id="wBal">৳1120</div><div class="progressWrap"><div id="wProgress" class="progressBar"></div></div></div><div class="card"><div style="font-weight:800;margin-bottom:12px">Withdraw Method</div><div id="bCard" class="payCard active" onclick="selectPay('bKash')"><div style="width:56px;height:56px;background:#e2136e;border-radius:16px;display:flex;align-items:center;justify-content:center;font-weight:900">৳</div><div style="flex:1"><b>bKash</b><div style="font-size:12px;color:#94a3b8">Personal</div></div><div class="check">✓</div></div><div id="nCard" class="payCard" onclick="selectPay('Nagad')"><div style="width:56px;height:56px;background:#f59e0b;border-radius:16px;display:flex;align-items:center;justify-content:center;font-weight:900">৳</div><div style="flex:1"><b>Nagad</b><div style="font-size:12px;color:#94a3af">Fast</div></div><div class="check">✓</div></div><input id="accNum" placeholder="01XXX"><input id="wdAmt" type="number" placeholder="500"><button class="btn" style="background:linear-gradient(90deg,#e2136e,#f59e0b)" onclick="doWithdraw()">Withdraw করুন</button></div></div>
<div id="p-support" class="page"><div class="card"><h3>Support</h3><button class="btn" style="background:#0ea5e9" onclick="openLink('tele_sup')">Telegram Support</button><button class="btn" style="background:#22c55e" onclick="openLink('wa_sup')">WhatsApp</button><button class="btn" style="background:#f59e0b" onclick="openLink('tutorial')">Tutorial Video</button></div></div>
...
<div style="margin-top:12px"><b style="font-size:22px" id="pName">User</b><div style="font-size:13px" id="pId">ID:</div><div id="memberBadge" style="display:inline-block;padding:7px 16px;border-radius:20px;font-size:13px;font-weight:800;margin-top:8px">🏅 1 Bronze Member</div></div>
...
let taskHtml=''; for(let i=1;i<=5;i++){ let done=u.tasks.includes(i); taskHtml+=`<div style="display:flex;align-items:center;justify-content:space-between;background:#0e0e20;border:1px solid #1e1e3a;border-radius:16px;padding:12px;margin:10px 0"><div><b>Task ${i}</b><div style="font-size:11px;color:#9ca3af">Task ${i} Complete = 20TK</div></div><button class="btn" style="width:auto;background:${done?'#22c55e':'#6d4cff'};padding:10px 18px;margin:0" onclick="doTask(${i})">${done?'✓':'20৳'}</button></div>`; } document.getElementById('taskList').innerHTML=taskHtml;
});}
...
function openLink(k){ let map={tele_sup:settings.tele_sup_link,wa_sup:settings.wa_sup_link,tutorial:settings.tutorial_youtube_link}; if(map[k]) window.open(map[k],'_blank');}
...
function saveAll(){ let data={}; document.querySelectorAll('input').forEach(e=>{ if(e.id) data[e.id]=e.value; }); fetch('/api/admin/save',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(data)}).then(r=>r.json()).then(d=>{ document.getElementById('msg').innerText=d.msg; alert('✅ রং সহ সব সেভ'); });}
load();
</script></body></html>
"""
