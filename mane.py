# -*- coding: utf-8 -*-
# FINAL FIXED - A to Z - VIDEO + GALLERY + PROFILE + ZONE 3490663
import os, json
...
<div id="p_home" class="page active">
  <div class="card"><h3 id="balTitle">আপনার বর্তমান ব্যালেন্স</h3><h1 style="font-size:32px;margin:8px 0">৳<span id="bal">0</span></h1><div style="display:flex;gap:10px"><div style="flex:1;background:#15152a;border-radius:10px;padding:8px;text-align:center"><small>Company</small><br><b><span id="adC">0</span>/30</b></div><div style="flex:1;background:#15152a;border-radius:10px;padding:8px;text-align:center"><small>Popup</small><br><b><span id="popC">0</span>/20</b></div></div>
  <button class="btn" onclick="showAd('company')">📺 Company Ad দেখুন (৳2)</button>
  <button class="btn" style="background:linear-gradient(90deg,#f59e0b,#ef4444)" onclick="showAd('popup')">🎁 Popup Ad (৳3)</button>
  </div>
  <div class="card" id="offerBox"></div>
</div>
...
<div id="p_profile" class="page">
  <div class="card" style="text-align:center">
    ...
    <div style="margin-top:12px;text-align:left"><small>ID: <span id="uid"></span></small><br><small>Join: <span id="join"></span></small></div>
    <div style="margin-top:12px"><a id="tutLink" target="_blank" style="color:#6d4cff">📺 Tutorial Video দেখুন</a></div>
  </div>
</div>
<div id="p_with" class="page"><div class="card"><h3>💸 Withdraw</h3><input id="wNum" placeholder="bKash/Nagad Number"><input id="wAmt" type="number" placeholder="Amount"><button class="btn" onclick="doWithdraw()">Withdraw করুন</button><div id="wList"></div></div></div>
<div class="btm">
  <div class="on" onclick="nav('home',this)"><span>🏠</span>Home</div>
  <div onclick="nav('tasks',this)"><span>📋</span>Tasks</div>
  <div onclick="nav('profile',this)"><span>👤</span>Profile</div>
  <div onclick="nav('with',this)"><span>💸</span>Withdraw</div>
</div>
<script>
let uid = new URLSearchParams(location.search).get('id') || '12345';
let sData=null;
...
function load(){fetch('/api/get?id='+uid).then(r=>r.json()).then(d=>{
 sData=d.settings; let u=d.user;
 document.getElementById('bal').innerText=u.balance; document.getElementById('adC').innerText=u.ads_today; document.getElementById('popC').innerText=u.popup_today;
 document.getElementById('uid').innerText=u.id; document.getElementById('join').innerText=u.join_date; document.getElementById('appName').innerText=sData.app_name; document.getElementById('balTitle').innerText=sData.balance_title;
 document.getElementById('tutLink').href=sData.tutorial_video;
 if(u.profile_img){document.getElementById('bigP').innerHTML='<img src="'+u.profile_img+'">'; document.getElementById('topP').innerHTML='<img src="'+u.profile_img+'">';}
 if(u.name) document.getElementById('nameIn').value=u.name;
 document.getElementById('offerBox').innerHTML='<h3>'+sData.offer_title+'</h3><p>'+sData.offer_desc+'</p>';
 let tl=''; d.tasks.forEach(t=>{tl+=`<div class="taskCard"><div><b>${t.icon} ${t.title}</b><br><small>${t.desc}</small></div><button class="btn" style="width:auto;padding:8px 14px" onclick="doTask(${t.id},'${t.link}')">${t.reward}৳</button></div>`}); document.getElementById('taskList').innerHTML=tl;
});}
function showAd(type){
 if(typeof show_3490663==='function'){
   if(type==='popup'){ show_3490663({type:'popup'}).then(()=>{ fetch('/api/reward?id='+uid+'&type=popup').then(()=>load()); }); }
   else { show_3490663().then(()=>{ fetch('/api/reward?id='+uid+'&type=company').then(()=>load()); }); }
 }else{ alert('Ad Loading... 3 sec por try korun'); }
}
...
ADMIN_HTML = """<!DOCTYPE html><html><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1"><style>body{background:#111;color:#fff;font-family:system-ui;padding:12px}input{width:100%;padding:10px;margin:6px 0;border-radius:8px;border:1px solid #333;background:#222;color:#fff}.btn{padding:12px;border:none;border-radius:10px;background:#6d4cff;color:#fff;font-weight:800;width:100%;margin-top:10px}</style></head><body>
<h2>👑 Admin Panel - Zone 3490663</h2>
<div id="form"></div>
<button class="btn" onclick="save()">Save All</button>
<script>
let sid='8807178385';
fetch('/api/get?id='+sid).then(r=>r.json()).then(d=>{
 let s=d.settings; let h='';
 h+=`<input id="app_name" value="${s.app_name}" placeholder="App Name">`;
 h+=`<input id="tutorial_video" value="${s.tutorial_video}" placeholder="Tutorial Video Link">`;
 h+=`<input id="offer_title" value="${s.offer_title}">`;
 h+=`<input id="offer_desc" value="${s.offer_desc}">`;
 h+=`<input id="bonus" value="${s.bonus}">`;
 h+=`<input id="ad_reward" value="${s.ad_reward}">`;
 h+=`<input id="popup_reward" value="${s.popup_reward}">`;
 h+=`<input id="company_limit" value="${s.company_limit}">`;
 h+=`<input id="popup_limit" value="${s.popup_limit}">`;
 h+=`<input id="min_with" value="${s.min_with}">`;
 d.tasks.forEach(t=>{h+=`<hr><b>Task ${t.id}</b><input id="task_${t.id}_title" value="${t.title}"><input id="task_${t.id}_reward" value="${t.reward}"><input id="task_${t.id}_link" value="${t.link}">`});
 document.getElementById('form').innerHTML=h;
});
function save(){
 let o={}; document.querySelectorAll('input').forEach(i=>{o[i.id]=i.value;});
 fetch('/api/admin/save',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(o)}).then(r=>r.json()).then(d=>alert(d.msg));
}
</script></body></html>
"""
