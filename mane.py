...
def load_db():
    if not os.path.exists(DB_FILE):
        return {"users": {}, "settings": {
            ...
            "refer_bottom_title": "🔥 Refer Bonus", "refer_bottom_desc": "Admin থেকে লিখতে পারবে", "refer_bottom_btn": "Join Now", "refer_bottom_link": "https://google.com",
            "sup_title": "Support Center", "sup_desc": "24/7 Support", "sup_tg": "https://t.me/", "sup_wa": "https://wa.me/", "sup_fb": "https://facebook.com", "sup_email": "support@bd.com",
            "sup_notice": "রাত ১০টার পর Withdraw বন্ধ", "sup_faq1_q": "Withdraw কতক্ষণে?", "sup_faq1_a": "24 ঘন্টা", "sup_faq2_q": "Refer টাকা?", "sup_faq2_a": "Instant", "sup_faq3_q": "Ad আসে না?", "sup_faq3_a": "AdBlock বন্ধ করুন",
            "sup_rules": "No Fake Account", "support_bottom_title": "Update", "support_bottom_desc": "Admin Box", "support_bottom_btn": "Contact", "support_bottom_link": "https://t.me/",
            "profile_bottom_title": "VIP Membership", "profile_bottom_desc": "VIP হলে বেশি ইনকাম", "profile_bottom_btn": "Upgrade", "profile_bottom_link": "https://google.com"
        }, "tasks": [
...
<div id='p-home' class='page active'>
  ...
  <div class='spon-big'><h2 style='font-size:28px'>🔥 {{s.spon_title}}</h2><p style='font-size:20px;margin-top:14px;line-height:1.5'>{{s.spon_desc}}</p><button class='btn' style='background:#f59e0b;color:#000;margin-top:20px;padding:18px' onclick="window.open('{{s.spon_link}}')">{{s.spon_btn}}</button></div>
</div>
...
function doWD(){let n=document.getElementById('wNum').value;let a=document.getElementById('wAmt').value; if(!n||!a) return alert('Number'); fetch('/api/wd',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({id:uid,amt:a,num:n,m:curMeth})}).then(r=>r.json()).then(d=>alert(d.msg));}
...
function init(){fetch('/api/init',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({id:uid})}).then(r=>r.json()).then(d=>{let u=d.user; let bal='৳'+u.bal; document.getElementById('balShow').innerText=bal; document.getElementById('balTopRight').innerText=bal; document.getElementById('balTopBalance').innerText=bal; let t1=document.getElementById('balTopTasks'); if(t1) t1.innerText=bal; let t2=document.getElementById('balTopRefer'); if(t2) t2.innerText=bal; let t3=document.getElementById('balTopProfile'); if(t3) t3.innerText=bal; document.getElementById('diamondTop').innerText=u.diamonds; document.getElementById('pNameShow').innerText=u.name; document.getElementById('pDiamond').innerText=u.diamonds; document.getElementById('pBal2').innerText=u.bal; document.getElementById('statEarned').innerText=bal; document.getElementById('statBal').innerText=bal; document.getElementById('statAds').innerText=(u.c_today||0)+(u.p_today||0); document.getElementById('statRefer').innerText=u.refl?u.refl.length:0; document.getElementById('accId').innerText=u.id; document.getElementById('accNameInput').value=u.name; document.getElementById('cToday').innerText=(u.c_today||0)+'/{{s.clim}}'; document.getElementById('pToday').innerText=(u.p_today||0)+'/{{s.plim}}'; document.getElementById('adsCount').innerText=(u.c_today||0)+(u.p_today||0)+'/80'; document.getElementById('refLinkBox').innerText=location.origin+'/?ref='+u.id; document.getElementById('refCount').innerText=u.refl?u.refl.length:0; if(u.photo){document.getElementById('proImg').src=u.photo; document.getElementById('proImg').style.display='block'; document.getElementById('proEmoji').style.display='none';} if(d.wds && d.wds.length>0){document.getElementById('wdHistList').innerHTML=d.wds.map(w=>`<div style="padding:10px;background:#0B0E1C;border-radius:10px;margin-top:6px;display:flex;justify-content:space-between"><div>${w.m} ${w.num}</div><div>৳${w.amt} ${w.st}</div></div>`).join('');}});} init(); loadTasks();
</script></body></html>
"""
...
def home():
    db=load_db()
    return render_template_string(HTML, s=db["settings"])
...
def admin_save():
    ...
    save_db(db)
    return "<script>alert('Saved - Big File Fixed!'); location.href='/admin';</script>"
...
def api_ads():
    ...
    save_db(db)
    return jsonify({"msg":f"✅ ৳{reward} Added! Ad দেখেছো!"})
...
def api_wd():
    ...
    u["bal"]-=amt; db.setdefault("wds",[]).append({"uid":uid,"amt":amt,"num":d["num"],"m":d["m"],"st":"Pending","time":datetime.now().strftime("%Y-%m-%d")}); save_db(db); return jsonify({"msg":"✅ Withdraw Sent!"})
