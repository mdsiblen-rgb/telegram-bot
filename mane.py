# -*- coding: utf-8 -*-
# FINAL A-Z FULL FILE - Protidiner Kaj BD
# Features: Thin Balance + Blink Banner + Profile Upload + Task Page + Bottom 3 Boxes + Full Admin Control
import os, json
...
@app.route('/admin')
def admin_page():
    if request.args.get('id')!='8807178385': return "Admin only? use?id=8807178385",403
...
<div class="card" style="text-align:center;background:linear-gradient(135deg,#1e3a8a,#2563eb,#06b6d4,#10b981);padding:20px 16px;border:1px solid rgba(255,255,255,0.18)">
<div class="shine"></div>
<div style="font-size:11px;color:#c7d2fe;font-weight:600;position:relative;z-index:3" id="balTitle">আপনার বর্তমান ব্যালেন্স</div>
<div style="font-size:42px;font-weight:500;margin:8px 0;position:relative;z-index:3">৳<span id="bal" style="font-weight:700">0</span></div>
<div style="display:flex;justify-content:center;gap:6px;flex-wrap:wrap;position:relative;z-index:3"><span style="background:rgba(0,0,0,0.3);padding:5px 10px;border-radius:20px;font-size:11px">Company <b id="ads" style="color:#fde68a">0</b>/<span id="adsLim">30</span></span><span style="background:rgba(0,0,0,0.3);padding:5px 10px;border-radius:20px;font-size:11px">Popup <b id="pop" style="color:#86efac">0</b>/<span id="popLim">20</span></span><span style="background:rgba(0,0,0,0.3);padding:5px 10px;border-radius:20px;font-size:11px">Total <b id="total">0</b></span></div>
<div style="background:rgba(0,0,0,0.35);height:6px;border-radius:20px;margin-top:14px;overflow:hidden;position:relative;z-index:3"><div id="prog" style="height:100%;background:linear-gradient(90deg,#fde68a,#fbbf24);width:0%;transition:width 1s"></div></div>
</div>
<div class="card"><button class="btn" style="background:linear-gradient(90deg,#6d4cff,#3a1aff)" onclick="watchAd()">📺 COMPANY ADS (৳<span id="r1">2</span>) - <span id="ads2">0</span>/<span id="adsLim2">30</span></button><button class="btn" style="background:linear-gradient(90deg,#00c853,#009624)" onclick="watchPop()">💰 POPUP ADS (৳<span id="r2">3</span>) - <span id="pop2">0</span>/<span id="popLim2">20</span></button><button class="btn" style="background:#1e293b" onclick="nav('task')">📋 TASK BONUS দেখুন</button></div>
...
<div style="margin-top:12px;background:linear-gradient(135deg,#0ea5e9,#0284c7);border-radius:16px;padding:16px;display:flex;justify-content:space-between;align-items:center"><div><div style="font-weight:800">📢 আমাদের Telegram</div><div style="font-size:11px;opacity:0.9">সকল আপডেট পেতে জয়েন করুন</div></div><button class="btn" style="width:auto;background:#fff;color:#0284c7;padding:10px 16px;margin:0" onclick="openTG()">Join ✈️</button></div>
<div style="margin-top:12px;background:#1a1a2e;border:1px solid #2a2a4a;border-radius:16px;padding:14px"><div style="font-weight:800;font-size:13px">⚠️ নিয়মাবলী</div><div style="font-size:11px;opacity:0.7;margin-top:6px;line-height:1.6">• 1 ফোনে 1 অ্যাকাউন্ট<br>• VPN ব্যবহার করবেন না<br>• 500 টাকা হলেই Withdraw<br>• প্রতিদিন Ads + Task = বেশি ইনকাম</div></div>
</div>
</div>

<div id="p-wallet" class="page"><div class="card"><h3>💰 Wallet</h3><p>Balance: ৳<span id="bal2">0</span></p><p style="font-size:11px;opacity:0.6;margin-top:6px">Min Withdraw ৳500 - bKash/Nagad</p></div></div>
...
<script>
let uid=new URLSearchParams(location.search).get('id')||'8807178385';let gAds=[],banners=[],idx=0,currentTasks=[],userData=null,settings=null;
...
async function load(){
 ...
 r1.innerText=s.ad_reward;r2.innerText=s.popup_reward;appName.innerText=s.app_name;adminName.innerText='Admin: '+s.admin_name;appLogo.innerText=s.app_logo;balTitle.innerText='💰 '+s.balance_title;offerTitle.innerText=s.offer_title;offerDesc.innerText=s.offer_desc;supportText.innerText='Support: '+(s.support_link||'');
...
ADMIN="""
<html><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><style>body{background:#0f0f0f;color:#fff;font-family:sans-serif;padding:14px}input,textarea{width:100%;padding:10px;margin:5px 0;border-radius:8px;border:1px solid #333;background:#1a1a1a;color:#fff}.card{background:#1e1e1e;padding:14px;border-radius:12px;margin:10px 0}.btn{padding:12px;width:100%;border:none;border-radius:8px;background:#6d4cff;color:#fff;font-weight:800;margin-top:6px;cursor:pointer}.prev{width:100%;height:120px;border-radius:10px;object-fit:cover;margin-top:6px;border:1px solid #333;display:none}.prev2{width:70px;height:70px;border-radius:50%;object-fit:cover;display:none}</style></head><body>
<h2>👑 Admin - Full Control A-Z</h2>
<div class="card" style="border:1.5px solid #f59e0b"><h3>⭐ বড় ব্যানার - ফটো (মিট মিট)</h3><input type="file" id="bannerFile" accept="image/*"><img id="bannerPrev" class="prev"><button class="btn" style="background:#f59e0b" onclick="uploadBanner()">📸 ব্যানার যোগ (5 টা পর্যন্ত)</button><button class="btn" style="background:#ef4444" onclick="clearBanner()">🗑️ সব মুছুন</button><div id="bannerCount" style="font-size:11px;margin-top:6px"></div></div>
<div class="card" style="border:1.5px solid #6d4cff"><h3>👤 প্রোফাইল ছবি (52px বড়)</h3><input type="file" id="fileIn" accept="image/*"><img id="preview" class="prev2"><button class="btn" style="background:#00c853" onclick="uploadImg()">📤 গ্যালারি থেকে আপলোড</button></div>
<div class="card"><h3>📝 নাম / লোগো</h3>App Name:<input id="app_name">Admin Name:<input id="admin_name">Logo Emoji:<input id="app_logo">TG Channel:<input id="tg_channel">Support Link:<input id="support_link">Balance Title:<input id="balance_title"></div>
<div class="card"><h3>📢 উপরের লেখা</h3>Ad 1:<input id="g1">Ad 2:<input id="g2">Ad 3:<input id="g3"></div>
<div class="card" style="border:1.5px solid #00c853"><h3>📋 Task Page - এডমিন থেকে সব চেঞ্জ</h3><div id="taskAdmin"></div></div>
<div class="card"><h3>🎁 অফার বক্স</h3>Title:<input id="offer_title">Desc:<textarea id="offer_desc"></textarea></div>
<div class="card"><h3>💰 লিমিট ও টাকা</h3>Company Limit:<input id="company_limit" type="number">Company Reward:<input id="ad_reward" type="number">Popup Limit:<input id="popup_limit" type="number">Popup Reward:<input id="popup_reward" type="number">Task Limit:<input id="task_limit" type="number"></div>
<button class="btn" style="background:#6d4cff;padding:16px;font-size:15px" onclick="save()">💾 Save All - সাথে সাথে আপডেট</button><div id="msg" style="text-align:center;margin-top:10px;color:#fde047;font-weight:800"></div>
<script>
let base64="",banner64="";
