# -*- coding: utf-8 -*-
# FINAL A-Z CLEAN - 1 to 5 Pages - Number Lock Text + bKash Nagad Logo Admin Control
import os, json
...
def default_data():
    return {
        ...
        "settings":{
            ...
            "task_limit":5,
            "min_with":500,
            "bkash_logo":"https://i.ibb.co/0jZzXQ0/bkash.png",
            "nagad_logo":"https://i.ibb.co/XYZ/nagad.png",
...
def load_db():
    ...
    try:
        with open(DB,'r',encoding='utf-8') as f:
            db=json.load(f)
            # migration for new keys
            if "bkash_logo" not in db["settings"]:
                db["settings"]["bkash_logo"]="https://i.ibb.co/0jZzXQ0/bkash.png"
                db["settings"]["nagad_logo"]="https://i.ibb.co/XYZ/nagad.png"
...
def task_complete():
    ...
    return jsonify({"msg":f"✅ {task['title']} - ৳{task['reward']}"})
...
<style>
*{box-sizing:border-box;margin:0;padding:0;font-family:system-ui}body{background:#070710;color:#fff;max-width:430px;margin:0 auto;padding-bottom:120px}
...
.payCard.active{border-color:#e2136e;background:#1a1a35}.payLogo{width:48px;height:48px;border-radius:10px;display:flex;align-items:center;justify-content:center;font-weight:900;color:#fff;overflow:hidden}
...
#lockOverlay{display:flex;background:linear-gradient(135deg,#0e0e20f2,#1a1a40f2)}#welcomeOverlay{background:linear-gradient(135deg,#6d28d9f2,#4f46e5f2)}#adOverlay{background:#000e;flex-direction:column;color:#fff}
input{width:100%;padding:12px;border-radius:10px;border:1px solid #333;background:#0e0e20;color:#fff;margin-top:8px}
</style></head><body>
<div class="top"><div><b>💎 Protidiner Kaj BD</b><div style="font-size:10px;color:#aaa" id="lockInfoTop">A-Z 1-5 Clean</div></div><div style="width:40px;height:40px;border-radius:50%;background:#222;display:flex;align-items:center;justify-content:center" onclick="goPage('profile')">👤</div></div>
...
<p style="font-size:13px;color:#333;font-weight:700">নাম্বার দিয়ে লক করুন, নাম্বার দিয়ে লগইন করুন</p>
<p style="font-size:11px;color:#666;margin-top:4px">আপনার একাউন্ট সুরক্ষিত থাকবে, একই নাম্বারে টাকা যাবে</p>
...
<input id="otpInput" type="text" placeholder="OTP দিন - 1234" maxlength="4" style="background:#f5f3ff;color:#000">
<button class="btn" onclick="verifyOTP()">✅ ভেরিফাই করুন</button>
...
<div class="card" style="background:linear-gradient(135deg,#6d28d9,#4f46e5)"><div style="font-size:12px;opacity:.9">আপনার বর্তমান ব্যালেন্স</div><div style="font-size:34px;font-weight:800" id="balMain">0 TK</div><div style="font-size:11px;background:#fff2;padding:4px 10px;border-radius:20px;display:inline-block;margin-top:6px" id="uidShow">ID: -</div></div>
...
<div class="card"><h3>📢 Google Popup Ads</h3><button class="btn" style="background:#0ea5e9" onclick="watchAd('popup')">👁️ Popup Ad দেখুন</button><p id="popText" style="text-align:right;font-size:11px;color:#aaa;margin-top:6px">0/20</p></div>
<div class="card"><h3>🔗 রেফার লিংক</h3><div id="refLink" style="background:#0e0e20;border:1px dashed #6d4cff;padding:10px;border-radius:10px;word-break:break-all;font-size:12px;margin-top:6px"></div><button class="btn" style="background:#fbbf24;color:#000" onclick="copyRef()">📋 কপি করুন</button></div>
</div>

<div id="p-tasks" class="page"><div class="card"><h3>🎯 Page 2 - ডেইলি টাস্ক</h3><div id="taskList"></div></div></div>

<div id="p-invite" class="page"><div class="card" style="background:linear-gradient(135deg,#6d28d9,#4f46e5);text-align:center"><h1 style="font-size:44px">50 TK</h1><p>Page 3 - প্রতি রেফারে</p><div id="refLink2" style="background:#fff2;padding:10px;border-radius:10px;margin-top:10px;font-size:12px;word-break:break-all"></div><button class="btn" style="background:#fff;color:#6d4cff" onclick="copyRef()">Invite কপি</button></div></div>
...
<div class="card"><h3>🏦 টাকা তুলুন</h3>
...
<div id="nagadOpt" class="payCard" onclick="selectMethod('Nagad')"><div class="payLogo" style="background:#f6921e"><img id="nagadImg" src="" style="display:none"><span id="nagadTxt">নগদ</span></div><div style="flex:1"><b id="nagadName">Nagad Personal</b><div style="font-size:10px;color:#aaa">লক নাম্বারে যাবে</div></div></div>
<input id="accNum" readonly><input id="amount" type="number" placeholder="Amount - Min 500"><button class="btn" onclick="doWithdraw()">💸 Withdraw করুন</button><div id="withdrawHistory" style="margin-top:10px"></div></div>
</div>

<div id="p-profile" class="page"><div class="card" style="text-align:center"><div style="width:80px;height:80px;border-radius:50%;background:#222;display:flex;align-items:center;justify-content:center;margin:0 auto;font-size:32px" id="profileBox">👤</div><h3 id="uName" style="margin-top:10px">User</h3><p style="font-size:11px;color:#aaa">ID: <span id="uidShow2">-</span></p><div style="background:#0e0e20;padding:10px;border-radius:10px;margin-top:10px;font-size:12px;text-align:left"><b>🔒 Locked Number:</b> <span id="profilePhone">-</span><br><b>💰 Bonus:</b> <span id="profileBonus">1120 TK</span></div></div></div>
...
<div class="btm"><div id="nav-home" class="on" onclick="goPage('home')"><span>🏠</span>Home</div><div id="nav-tasks" onclick="goPage('tasks')"><span>🎯</span>Tasks</div><div id="nav-invite" onclick="goPage('invite')"><span>👥</span>Invite</div><div id="nav-wallet" onclick="goPage('wallet')"><span>💰</span>Wallet</div><div id="nav-profile" onclick="goPage('profile')"><span>👤</span>Profile</div></div>

<script>
let uid=localStorage.getItem('locked_phone')||''; let selected='bKash'; let settings={}; let user={};
function initApp(){ if(!uid){ document.getElementById('lockOverlay').style.display='flex'; return; } document.getElementById('lockOverlay').style.display='none'; fetch('/api/get?id='+uid).then(r=>r.json()).then(d=>{ settings=d.settings; user=d.user; document.getElementById('balMain').innerText=user.balance+' TK'; document.getElementById('bal2').innerText=user.balance+' TK'; document.getElementById('uidShow').innerText='ID: '+uid; document.getElementById('uidShow2').innerText=uid; document.getElementById('lockedNumShow').innerText=uid; document.getElementById('profilePhone').innerText=uid; document.getElementById('welcomePhone').innerText=uid; document.getElementById('accNum').value=uid; document.getElementById('uName').innerText=user.name; document.getElementById('bonusBox').innerText=settings.bonus+' TK বোনাস 💰'; document.getElementById('bonusText').innerText=settings.bonus+' TK'; document.getElementById('profileBonus').innerText=settings.bonus+' TK'; document.getElementById('bkashName').innerText=settings.bkash_name; document.getElementById('nagadName').innerText=settings.nagad_name; if(settings.bkash_logo && settings.bkash_logo.startsWith('http')){ document.getElementById('bkashImg').src=settings.bkash_logo; document.getElementById('bkashImg').style.display='block'; document.getElementById('bkashTxt').style.display='none'; } if(settings.nagad_logo && settings.nagad_logo.startsWith('http')){ document.getElementById('nagadImg').src=settings.nagad_logo; document.getElementById('nagadImg').style.display='block'; document.getElementById('nagadTxt').style.display='none'; } document.getElementById('adText').innerText=user.ads_today+'/'+settings.company_limit; document.getElementById('popText').innerText=user.popup_today+'/'+settings.popup_limit; document.getElementById('refLink').innerText=window.location.origin+'?start='+uid; document.getElementById('refLink2').innerText=window.location.origin+'?start='+uid; let tHtml=''; d.tasks.forEach(t=>{ let done=user.tasks_done.includes(t.id); tHtml+=`<div style="display:flex;justify-content:space-between;align-items:center;background:#0e0e20;padding:10px;border-radius:10px;margin:8px 0"><div><b>${t.icon} ${t.title}</b><div style="font-size:10px;color:#aaa">${t.desc}</div></div><button class="btn" style="width:auto;padding:8px 12px;background:${done?'#10b981':'#6d4cff'}" onclick="doTask(${t.id})">${done?'✓':t.reward+' TK'}</button></div>`; }); document.getElementById('taskList').innerHTML=tHtml; if(!localStorage.getItem('welcomed_'+uid)){ document.getElementById('welcomeOverlay').style.display='flex'; } }); }
function sendOTP(){ let p=document.getElementById('phoneInput').value; if(p.length!=11||!p.startsWith('01')){ alert('সঠিক 11 ডিজিট দিন - 01XXXXXXXXX'); return; } alert('✅ OTP: 1234\\nNumber: '+p); document.getElementById('otpSection').style.display='block'; document.getElementById('sendBtn').innerText='🔁 আবার পাঠান'; }
function verifyOTP(){ let o=document.getElementById('otpInput').value; let p=document.getElementById('phoneInput').value; if(o!='1234'){ alert('ভুল OTP! 1234'); return; } localStorage.setItem('locked_phone',p); uid=p; document.getElementById('lockOverlay').style.display='none'; fetch('/api/get?id='+p).then(()=>{ initApp(); document.getElementById('welcomeOverlay').style.display='flex'; }); }
...
ADMIN_HTML = """
<!DOCTYPE html><html lang="bn"><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1.0"><title>Admin - Clean</title>
...
<h2>👑 Admin Panel - Clean A-Z - Logo Control</h2>
<div class="card"><h3>⚙️ Bonus & General Settings</h3>
App Name: <input id="app_name"><br>Bonus: <input id="bonus" type="number"><br>Ad Reward: <input id="ad_reward" type="number"><br>Min Withdraw: <input id="min_with" type="number"><br>
<button class="btn" onclick="save()">💾 Save Settings</button></div>

<div class="card"><h3>🖼️ bKash / Nagad Logo Change - এডমিন থেকে পরিবর্তন</h3>
<p style="font-size:11px;color:#aaa">আপনি এখান থেকে লোগোর লিংক পরিবর্তন করতে পারবেন - সাথে নামও</p>
bKash Name: <input id="bkash_name" placeholder="bKash Personal"><br>
bKash Logo URL: <input id="bkash_logo" placeholder="https://.../bkash.png - সরাসরি ছবির লিংক"><br>
...
<div style="display:flex;gap:8px;margin-top:8px"><div style="flex:1;text-align:center"><div>bKash Preview</div><img id="prevBkash" style="width:60px;height:60px;object-fit:cover;border-radius:10px;background:#e2136e;margin-top:6px"></div><div style="flex:1;text-align:center"><div>Nagad Preview</div><img id="prevNagad" style="width:60px;height:60px;object-fit:cover;border-radius:10px;background:#f6921e;margin-top:6px"></div></div>
<button class="btn" onclick="save()">💾 Logo Save করুন</button>
<p style="font-size:10px;color:#aaa;margin-top:6px">Tip: imgbb.com এ ছবি আপলোড করে Direct Link কপি করে এখানে বসান</p>
</div>

<div class="card"><h3>💸 Balance Add/Minus</h3><input id="admPhone" placeholder="Phone 01XXXXXXXXX"><input id="admAmount" type="number" placeholder="Amount"><div style="display:flex;gap:8px"><button class="btn" style="background:#10b981" onclick="bal('add')">+ যোগ</button><button class="btn" style="background:#ef4444" onclick="bal('minus')">- মাইনাস</button></div><div id="admMsg" style="font-size:12px;margin-top:6px"></div></div>
...
<script>
let db={};
function load(){ fetch('/api/get?id=8807178385').then(r=>r.json()).then(d=>{ db=d; let s=d.settings; document.getElementById('app_name').value=s.app_name||''; document.getElementById('bonus').value=s.bonus||1120; document.getElementById('ad_reward').value=s.ad_reward||2; document.getElementById('min_with').value=s.min_with||500; document.getElementById('bkash_name').value=s.bkash_name||'bKash Personal'; document.getElementById('nagad_name').value=s.nagad_name||'Nagad Personal'; document.getElementById('bkash_logo').value=s.bkash_logo||''; document.getElementById('nagad_logo').value=s.nagad_logo||''; if(s.bkash_logo) document.getElementById('prevBkash').src=s.bkash_logo; if(s.nagad_logo) document.getElementById('prevNagad').src=s.nagad_logo; let uhtml='<table><tr><th>Phone</th><th>Name</th><th>Balance</th><th>Join</th></tr>'; Object.values(d.all_users).forEach(u=>{ uhtml+=`<tr><td>${u.phone}</td><td>${u.name}</td><td>${u.balance}</td><td>${u.join_date}</td></tr>`; }); uhtml+='</table>'; document.getElementById('userList').innerHTML=uhtml; let whtml='<table><tr><th>Phone</th><th>Amount</th><th>Method</th><th>Time</th></tr>'; d.all_withdraws.slice(-30).reverse().forEach(w=>{ whtml+=`<tr><td>${w.uid}</td><td>${w.amount}</td><td>${w.method}</td><td>${w.time}</td></tr>`; }); whtml+='</table>'; document.getElementById('withdrawList').innerHTML=whtml; }); }
