# -*- coding: utf-8 -*-
# FINAL PREMIUM FULL - ZONE 3490663 - NO EMPTY
import os, json
...
def default_data():
    return {
        "users":{}, "withdraws":[],
        "settings":{
            "app_name":"Protidiner Kaj BD","admin_name":"SHIBLI NOMAN","zone":"3490663",
            "bonus":1120,"ad_reward":2,"popup_reward":3,"company_limit":30,"popup_limit":20,"task_limit":5,"min_with":500,
            "official_banners":[],"google_ads":["🎉 Daily Bonus Available","⭐ Official Ad bKash Nagad Trusted","📢 Company Sponsored 100% Safe"],
            "offer_title":"🎉 স্পেশাল অফার","offer_desc":"50 টা Ads দেখলে বোনাস!","balance_title":"ব্যালেন্স",
        ...
        "tasks":[
            ...
            {"id":2,"title":"YouTube Subscribe","reward":30,"icon":"▶️","link":"https://youtube.com/","desc":"Subscribe করুন"},
            {"id":3,"title":"Facebook Page Like","reward":20,"icon":"👍","link":"https://facebook.com/","desc":"Like দিন"},
            {"id":4,"title":"Refer Friend","reward":50,"icon":"👨‍👩‍👧‍👦","link":"","desc":"1 জন রেফার = 50"},
...
def load_db():
    if not os.path.exists(DB):
        d=default_data(); open(DB,'w',encoding='utf-8').write(json.dumps(d,ensure_ascii=False,indent=2)); return d
...
def withdraw():
    ...
    if amt < s["min_with"]: return jsonify({"msg":f"Min {s['min_with']}৳"})
    if u["balance"]<amt: return jsonify({"msg":"ব্যালেন্স কম"})
    if len(num)<11: return jsonify({"msg":"নাম্বার ভুল"})
    u["balance"]-=amt; db["withdraws"].append({"uid":uid,"amount":amt,"number":num,"method":method,"status":"Pending","time":str(datetime.now())[:16]}); save_db(db)
    return jsonify({"msg":f"✅ {method} {amt}৳ Request Done"})
@app.route('/api/user/update',methods=['POST'])
def user_update():
    db=load_db(); j=request.json; u=get_user(db,str(j.get('id')))
    if 'name' in j: u["name"]=str(j['name'])[:25]
    if j.get('profile_img'): u["profile_img"]=j['profile_img']
