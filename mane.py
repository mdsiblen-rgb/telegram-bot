...
def withdraw():
    ...
    save_db(db)
    return jsonify({"msg":"✅ Withdraw রিকোয়েস্ট পাঠানো হয়েছে - 24 ঘণ্টার মধ্যে পাবেন"})
...
body{background:#000;color:#fff;max-width:430px;margin:0 auto;padding-bottom:110px}
...
.navItem{text-align:center;color:#6b7280;font-size:11px;cursor:pointer;line-height:1.2}.navItem.active{color:#fff}.navItem span{font-size:24px;display:block}
...
.pill{background:#00000035;padding:7px 14px;border-radius:20px;font-size:13px;font-weight:600;border:1px solid #ffffff15;display:inline-block}
...
<div style="background:linear-gradient(90deg,#ff9a00,#ff3d00);border-radius:24px;height:145px;margin:12px;display:flex;align-items:center;justify-content:center;font-weight:800;font-size:20px;text-align:center">🎉 Daily Bonus Available Today</div>
<div class="balanceCard"><div style="font-size:14px;color:#e0f2fe">💰 আপনার বর্তমান ব্যালেন্স</div><div style="font-size:56px;font-weight:900;margin:6px 0" id="balHome">৳1120</div><div style="display:flex;gap:8px;justify-content:center;flex-wrap:wrap"><span class="pill" id="bC">Company 0/30</span><span class="pill" id="bP">Popup 0/20</span><span class="pill" id="bT">Total 0</span></div><div style="width:100%;height:6px;background:#00000040;border-radius:10px;margin-top:14px"><div id="progBar" style="height:100%;background:#ff0000;width:35%;border-radius:10px"></div></div></div>
<div class="boxDark"><button class="btn" style="background:#6d28d9" onclick="doR('company')">📺 COMPANY ADS (৳<span id="cr">2</span>) - <span id="btnC">0/30</span></button><button class="btn" style="background:#16a34a" onclick="doR('popup')">💰 POPUP ADS (৳<span id="pr">3</span>) - <span id="btnP">0/20</span></button><button class="btn" style="background:#232336" onclick="goP('tasks')">📋 TASK BONUS - 5 টা/দিন</button></div>
...
<div style="background:linear-gradient(90deg,#0ea5e9,#0284c7);border-radius:20px;padding:16px;margin:12px;display:flex;justify-content:space-between;align-items:center"><div><b>📢 Telegram Channel</b><div style="font-size:12px">Join করুন - আপডেট পান</div></div><button style="background:#fff;color:#0284c7;border:none;padding:10px 16px;border-radius:20px;font-weight:900" onclick="window.open(teleLink,'_blank')">Join ✈️</button></div>
</div>

<!-- 2 - TASK -->
<div id="p-tasks" class="page">
<div class="boxDark"><div style="font-size:20px;font-weight:900;padding:8px">📋 Task Bonus - দিনে 5 টা</div>
<div class="rowCard"><div>✈️ Telegram Channel Join<br><small style="color:#9ca3af">চ্যানেলে জয়েন করুন</small></div><button style="background:#6d4cff;color:#fff;border:none;padding:10px 18px;border-radius:12px;font-weight:800" onclick="doTask('tele',25)">৳25</button></div>
<div class="rowCard"><div>▶️ YouTube Subscribe<br><small style="color:#9ca3af">সাবস্ক্রাইব + লাইক</small></div><button style="background:#6d4cff;color:#fff;border:none;padding:10px 18px;border-radius:12px;font-weight:800" onclick="doTask('yt',30)">৳30</button></div>
<div class="rowCard"><div>👍 Facebook Page Like<br><small style="color:#9ca3af">পেজে লাইক দিন</small></div><button style="background:#6d4cff;color:#fff;border:none;padding:10px 18px;border-radius:12px;font-weight:800" onclick="doTask('fb',20)">৳20</button></div>
<div class="rowCard"><div>👥 Refer Friend<br><small style="color:#9ca3af">১ জন রেফার = ৳50</small></div><button style="background:#6d4cff;color:#fff;border:none;padding:10px 18px;border-radius:12px;font-weight:800" onclick="goP('profile')">৳50</button></div>
<div class="rowCard"><div>✅ Daily Check-in<br><small style="color:#9ca3af">প্রতিদিন একবার</small></div><button style="background:#6d4cff;color:#fff;border:none;padding:10px 18px;border-radius:12px;font-weight:800" onclick="doTask('check',15)">৳15</button></div>
</div>
<div style="background:linear-gradient(135deg,#6d28d9,#7c3aed);border-radius:20px;padding:16px;margin:12px"><div style="font-weight:800">🎁 Refer & Earn ৳50</div><div style="background:#2a1a6a;border-radius:12px;padding:10px;font-size:12px;word-break:break-all;color:#c4b5fd;margin-top:10px" id="refLinkT">https://...</div><button style="background:#fff;color:#4c1d95;border:none;width:100%;padding:12px;border-radius:12px;font-weight:900;margin-top:12px" onclick="copyRefer()">📋 লিংক কপি করুন</button></div>
</div>

<!-- 3 - WALLET - FIXED EXACTLY LIKE YOUR LAST IMAGE -->
<div id="p-wallet" class="page">
<div class="walletBal">
<div style="color:#8b95a5">ব্যালেন্স</div>
<div style="font-size:72px;font-weight:900;margin:8px 0" id="wBal">৳1120</div>
<div style="color:#7a8594">Min ৳500</div>
</div>
<div class="wMethod">
<div style="font-size:20px;font-weight:900;margin-bottom:12px">💸 Withdraw Method</div>
<div class="mCard active" id="bkCard" onclick="selPay('bKash')"><div class="mIcon" style="background:#e2136e">৳</div><div style="flex:1"><div style="font-weight:700">bKash</div><div style="font-size:13px;color:#9ca3af">Personal • Instant Payment</div></div><div style="color:#22c55e;font-size:20px" id="bkCheck">✓</div></div>
<div class="mCard" id="ngCard" onclick="selPay('Nagad')"><div class="mIcon" style="background:#ff7a00">৳</div><div style="flex:1"><div style="font-weight:700">Nagad</div><div style="font-size:13px;color:#9ca3af">Personal • Fast Withdraw</div></div><div style="color:#22c55e;font-size:20px;display:none" id="ngCheck">✓</div></div>
<input id="accNum" class="inputF" placeholder="01XXXXXXXXXX">
<input id="wdAmt" class="inputF" type="number" placeholder="500">
<button class="wBtn" onclick="doWd()">🚀 Withdraw করুন</button>
</div>
<div style="background:linear-gradient(135deg,#065f46,#047857);border-radius:20px;padding:18px;margin:12px"><b>✅ Withdraw নিয়ম</b><div style="font-size:12px;margin-top:6px;line-height:1.8">• সর্বনিম্ন ৳500<br>• 24 ঘণ্টার মধ্যে পেমেন্ট<br>• ভুল নাম্বারে গেলে দায়ী নন</div></div>
<div class="boxDark"><b>📜 Withdraw History</b><div style="font-size:13px;color:#6b7280;text-align:center;padding:20px" id="wHist">কোনো Withdraw নেই</div></div>
</div>

<!-- 4 - SUPPORT -->
<div id="p-support" class="page">
<div style="background:linear-gradient(135deg,#6d4cff,#4f46e5);border-radius:24px;padding:18px;margin:12px;text-align:center"><div style="background:#00000025;border-radius:16px;padding:12px;margin-bottom:14px;font-weight:600">💬 যেকোনো প্রয়োজনে এডমিনের সাথে যোগাযোগ করুন</div><div style="font-size:50px">💬</div><div style="font-size:22px;font-weight:900;margin-top:8px">আমরা আছি আপনার পাশে</div><div style="font-size:13px;opacity:.9;margin-top:6px">২৪ ঘণ্টা সাপোর্ট • 100% Trusted • SHIBLI NOMAN Team</div></div>
<div class="boxDark"><div style="font-size:18px;font-weight:900;margin-bottom:12px">🚀 দ্রুত যোগাযোগ করুন</div>
<div class="rowCard" style="border-color:#0ea5e9" onclick="window.open(teleLink,'_blank')"><div style="display:flex;gap:12px;align-items:center"><div style="width:52px;height:52px;background:#0ea5e9;border-radius:14px;display:flex;align-items:center;justify-content:center;font-size:24px">✈️</div><div><b>Telegram Support (Fast Reply)</b><div style="font-size:12px;color:#9ca3af">2 মিনিটে রিপ্লাই • 9AM-12AM</div></div></div><div>➡️</div></div>
<div class="rowCard" onclick="openWa()"><div style="display:flex;gap:12px;align-items:center"><div style="width:52px;height:52px;background:#22c55e;border-radius:14px;display:flex;align-items:center;justify-content:center;font-size:24px">💬</div><div><b>WhatsApp Support</b><div style="font-size:12px;color:#9ca3af" id="waNum">01XXXXXXXXXX</div></div></div><div>➡️</div></div>
<div class="rowCard" onclick="openEmail()"><div style="display:flex;gap:12px;align-items:center"><div style="width:52px;height:52px;background:#ff8c00;border-radius:14px;display:flex;align-items:center;justify-content:center;font-size:24px">📧</div><div><b>Email Support</b><div style="font-size:12px;color:#9ca3af" id="emTxt">support@protidinerkajbd.com</div></div></div><div>➡️</div></div>
</div>
<div class="boxDark"><div style="font-size:20px;font-weight:900">🎥 কিভাবে কাজ করবেন?</div><div style="background:linear-gradient(135deg,#000,#1a0f00);border:2px solid #ff8c00;border-radius:18px;padding:24px;text-align:center;margin:12px 0;cursor:pointer" onclick="window.open(ytTut,'_blank')"><div style="width:80px;height:80px;background:#ff8c00;border-radius:14px;display:flex;align-items:center;justify-content:center;font-size:40px;margin:0 auto;box-shadow:0 0 20px #ff8c0088">▶️</div><div style="background:#ff8c00;color:#fff;padding:8px 16px;border-radius:20px;font-weight:800;display:inline-block;margin-top:14px">Tutorial - 2 মিনিটে শিখুন</div><div style="font-size:12px;color:#9ca3af;margin-top:8px">▶️ Click করলে ভিডিও চলবে</div></div><div style="font-size:14px;line-height:1.9;color:#cbd5e1">Step 1: Ads দেখুন<br>Step 2: Task complete করুন<br>Step 3: ৳500 হলেই Withdraw</div></div>
<div class="boxDark"><div style="font-size:22px;font-weight:900">❓ FAQ</div><div class="rowCard"><div><b>Q: টাকা কখন পাবো?</b><div style="font-size:13px;color:#9ca3af">A: 24 ঘণ্টার মধ্যে bKash/Nagad এ।</div></div></div><div class="rowCard"><div><b>Q: VPN চলবে?</b><div style="font-size:13px;color:#9ca3af">A: না, ব্যান হবে।</div></div></div><div class="rowCard"><div><b>Q: 1 ফোনে কয়টা একাউন্ট?</b><div style="font-size:13px;color:#9ca3af">A: 1 টা।</div></div></div><div class="rowCard"><div><b>Q: Refer বোনাস?</b><div style="font-size:13px;color:#9ca3af">A: 1 জন = ৳50 সাথে সাথে।</div></div></div></div>
<div style="background:linear-gradient(135deg,#065f46,#047857);border-radius:20px;padding:18px;margin:12px;text-align:center"><div style="font-size:18px;font-weight:900">🛡️ 100% Trusted</div><div style="font-size:13px;margin-top:6px">50k+ ইউজার, 100% পেমেন্ট গ্যারান্টি। সমস্যা হলে Telegram এ মেসেজ দিন।</div></div>
</div>

<!-- 5 - PROFILE -->
<div id="p-profile" class="page">
<div style="background:#131326;border-radius:24px;padding:20px;margin:12px;border:1px solid #1e1e3a;text-align:center"><div style="width:120px;height:120px;border-radius:50%;border:4px solid #6d4cff;margin:0 auto;background:#0a0a18;display:flex;align-items:center;justify-content:center;font-size:60px;overflow:hidden;cursor:pointer;position:relative" onclick="pickImg()" id="avatarBox">👤<div style="position:absolute;bottom:0;right:0;width:32px;height:32px;background:#6d4cff;border-radius:50%;display:flex;align-items:center;justify-content:center;border:2px solid #131326">📷</div></div><div style="margin-top:14px;font-size:20px;font-weight:800" id="pNameTop">User 8385</div><div style="font-size:13px;color:#9ca3af" id="pIdTop">ID: 8807178385</div><div style="background:#6d4cff;color:#fff;padding:6px 14px;border-radius:20px;font-size:13px;font-weight:700;display:inline-block;margin-top:8px">🏅 Bronze Member</div><input id="nameInput" style="width:100%;background:#0e0e20;border:1px solid #25253d;border-radius:14px;padding:14px;color:#fff;margin:10px 0;text-align:center" placeholder="আপনার নাম লিখুন"><button style="width:100%;background:#6d4cff;border:none;border-radius:14px;padding:16px;font-weight:800;color:#fff;cursor:pointer;margin:8px 0" onclick="pickImg()">📷 গ্যালারি থেকে ছবি নিন</button><button style="width:100%;background:#16a34a;border:none;border-radius:14px;padding:16px;font-weight:800;color:#fff;cursor:pointer;margin:8px 0" onclick="saveProfile()">💾 Save Profile</button><div style="font-size:11px;color:#6b7280;margin-top:6px">ছবিতে বা বাটনে ক্লিক → গ্যালারি খুলবে → Save দিন</div></div>
<div class="boxDark"><div style="font-size:20px;font-weight:900">📊 পরিসংখ্যান</div><div style="display:grid;grid-template-columns:1fr 1fr 1fr;gap:10px;margin-top:12px"><div style="background:#1a1a30;border:1px solid #25253d;border-radius:16px;padding:14px;text-align:center"><div style="font-size:28px">💰</div><div style="font-size:18px;font-weight:900" id="sBal">৳1120</div><div style="font-size:11px;color:#9ca3af">ব্যালেন্স</div></div><div style="background:#1a1a30;border:1px solid #25253d;border-radius:16px;padding:14px;text-align:center"><div style="font-size:28px">📺</div><div style="font-size:18px;font-weight:900" id="sAds">0</div><div style="font-size:11px;color:#9ca3af">Ads</div></div><div style="background:#1a1a30;border:1px solid #25253d;border-radius:16px;padding:14px;text-align:center"><div style="font-size:28px">📋</div><div style="font-size:18px;font-weight:900" id="sTask">0</div><div style="font-size:11px;color:#9ca3af">Task</div></div><div style="background:#1a1a30;border:1px solid #25253d;border-radius:16px;padding:14px;text-align:center"><div style="font-size:28px">👥</div><div style="font-size:18px;font-weight:900" id="sWork">0</div><div style="font-size:11px;color:#9ca3af">Total Work</div></div><div style="background:#1a1a30;border:1px solid #25253d;border-radius:16px;padding:14px;text-align:center"><div style="font-size:14px">📅 17</div><div style="font-size:14px;font-weight:900" id="sJoin">2026-09-13</div><div style="font-size:11px;color:#9ca3af">Join Date</div></div><div style="background:#1a1a30;border:1px solid #25253d;border-radius:16px;padding:14px;text-align:center"><div style="background:#a855f7;padding:2px 8px;border-radius:6px;font-size:12px;font-weight:800;display:inline-block">ID</div><div style="font-size:11px;font-weight:900;margin-top:6px" id="sId">8807178385</div><div style="font-size:11px;color:#9ca3af">User ID</div></div></div></div>
<div class="boxDark"><div style="font-size:20px;font-weight:900">⚙️ সেটিংস</div><div class="rowCard" onclick="goP('wallet')"><div style="display:flex;gap:12px;align-items:center"><div style="width:48px;height:48px;background:#1e3a5f;border-radius:12px;display:flex;align-items:center;justify-content:center">💸</div><div><b>Withdraw History</b><div style="font-size:12px;color:#9ca3af">আপনার পেমেন্ট দেখুন</div></div></div><div>➡️</div></div><div class="rowCard" onclick="copyRefer()"><div style="display:flex;gap:12px;align-items:center"><div style="width:48px;height:48px;background:#1e3a5f;border-radius:12px;display:flex;align-items:center;justify-content:center">🔗</div><div><b>My Refer Link</b><div style="font-size:11px;color:#9ca3af;word-break:break-all" id="refLinkP">https://...</div></div></div><div>📋</div></div><div class="rowCard" onclick="rateApp()"><div style="display:flex;gap:12px;align-items:center"><div style="width:48px;height:48px;background:#1e3a5f;border-radius:12px;display:flex;align-items:center;justify-content:center">⭐</div><div><b>App Rate করুন</b><div style="font-size:12px;color:#9ca3af">5 Star দিন</div></div></div><div>➡️</div></div></div>
<div style="background:linear-gradient(135deg,#065f46,#0a7a4a);border-radius:20px;padding:18px;margin:12px;text-align:center;border:1px solid #047857"><div style="font-size:18px;font-weight:900">🛡️ Verified User</div><div style="font-size:13px;margin-top:6px">আপনার একাউন্ট 100% Safe • 24h Support</div></div>
</div>
...
function init(){
 fetch('/api/get?id='+uid).then(r=>r.json()).then(d=>{
  ...
  if(u.img){document.getElementById('avatarBox').innerHTML=`<img src="${u.img}" style="width:100%;height:100%;object-fit:cover;border-radius:50%"><div style="position:absolute;bottom:0;right:0;width:32px;height:32px;background:#6d4cff;border-radius:50%;display:flex;align-items:center;justify-content:center;border:2px solid #131326">📷</div>`;}
 });
}
...
function handleFile(inp){ let f=inp.files[0]; if(!f) return; let r=new FileReader(); r.onload=function(e){ tempImg=e.target.result; document.getElementById('avatarBox').innerHTML=`<img src="${tempImg}" style="width:100%;height:100%;object-fit:cover;border-radius:50%"><div style="position:absolute;bottom:0;right:0;width:32px;height:32px;background:#6d4cff;border-radius:50%;display:flex;align-items:center;justify-content:center;border:2px solid #131326">📷</div>`; }; r.readAsDataURL(f); }
