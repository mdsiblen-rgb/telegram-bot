...
def load_db():
    if not os.path.exists(DB):
        d={"users":{},"withdraws":[],"settings":{
            "admin_name":"SHIBLI NOMAN","app_name":"Protidiner Kaj BD","balance_target":"2000",
            "company_limit":"30","company_reward":"2","email":"support@protidinerkajbd.com",
            "min_withdraw":"500","popup_limit":"20","popup_reward":"3",
            "tele_link":"https://t.me/","whatsapp":"01XXXXXXXXXX","yt_tutorial":"https://youtube.com/",
...
def admin_save():
    ...
    return jsonify({"msg":"✅ SAVE ALL হয়েছে"})
...
body{background:#0a0a14;color:#fff;max-width:500px;margin:0 auto;padding:0;font-family:system-ui}
.card{background:#16162a;border-radius:20px;padding:16px;margin:0}
input{width:100%;padding:14px 16px;border-radius:12px;background:#0e0e20;border:1px solid #2a2a45;color:#fff;margin-top:6px;font-size:15px;box-sizing:border-box}
label{font-size:12px;color:#8b8ba7;margin-top:14px;display:block}
.btn{width:100%;padding:16px;background:#6d4cff;border:none;border-radius:14px;color:#fff;font-weight:800;margin-top:20px;cursor:pointer;font-size:16px;display:flex;align-items:center;justify-content:center;gap:8px}
h2{text-align:center;padding:20px 0 10px;font-size:26px}
</style></head><body>
<h2>👑 SUPER ADMIN - All Control</h2>
<div class="card">
<label>admin_name</label><input id="admin_name" value="SHIBLI NOMAN">
<label>app_name</label><input id="app_name" value="Protidiner Kaj BD">
<label>balance_target</label><input id="balance_target" value="2000">
<label>company_limit</label><input id="company_limit" value="30">
<label>company_reward</label><input id="company_reward" value="2">
<label>email</label><input id="email" value="support@protidinerkajbd.com">
<label>min_withdraw</label><input id="min_withdraw" value="500">
<label>popup_limit</label><input id="popup_limit" value="20">
<label>popup_reward</label><input id="popup_reward" value="3">
<label>tele_link</label><input id="tele_link" value="https://t.me/">
<label>whatsapp</label><input id="whatsapp" value="01XXXXXXXXXX">
<label>yt_tutorial</label><input id="yt_tutorial" value="https://youtube.com/">
<button class="btn" onclick="saveAll()">💾 SAVE ALL</button>
</div>
<script>
function load(){fetch('/api/admin/get').then(r=>r.json()).then(d=>{let s=d.settings; for(let k in s){let e=document.getElementById(k); if(e) e.value=s[k];}});}
function saveAll(){let s={}; document.querySelectorAll('input').forEach(e=>s[e.id]=e.value); fetch('/api/admin/save',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({settings:s})}).then(r=>r.json()).then(d=>alert(d.msg));}
load();
</script></body></html>"""
...
function init(){
 fetch('/api/get?id='+uid).then(r=>r.json()).then(d=>{
  ...
  let link='https://telegram-bot-1-v77g.onrender.com/?ref='+u.id; document.getElementById('refLinkT').innerText=link; document.getElementById('refLinkP').innerText=link;
